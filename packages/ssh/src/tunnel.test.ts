...
import {
  ...
  buildRemoteWaypointRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote waypoint runner with npx and npm fallbacks", () => {
    const script = buildRemoteWaypointRunnerScript({
      nodeEngineRange: TEST_NODE_ENGINE_RANGE,
      packageSpec: "waypoint@latest",
    });

    assert.include(script, "WAYPOINT_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec waypoint "$@"');
    assert.include(script, 'exec "$WAYPOINT_CLI_PATH" "$@"');
    assert.include(script, "could not install 'waypoint@latest'");
    assert.include(script, "require_installed_waypoint_cli npx --yes --package 'waypoint@latest'");
    assert.include(
      script,
      "require_installed_waypoint_cli npm exec --yes --package 'waypoint@latest'",
    );
    assert.include(script, "npm produced no waypoint executable");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `WAYPOINT_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for WAYPOINT_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
  ...
  it("does not hard-code a remote node engine range", () => {
    ...
    assert.include(script, "WAYPOINT_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote waypoint runner", () => {
    const script = buildRemoteWaypointRunnerScript({
      packageSpec: "waypoint@nightly; touch /tmp/waypoint-owned",
    });

    assert.include(
      script,
      "require_installed_waypoint_cli npx --yes --package 'waypoint@nightly; touch /tmp/waypoint-owned'",
    );
    assert.notInclude(script, "exec npx --yes waypoint@nightly; touch /tmp/waypoint-owned");
  });

  it("builds the remote waypoint runner with a node script override", () => {
    const script = buildRemoteWaypointRunnerScript({
    ...
    assert.include(
      script,
      "WAYPOINT_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$WAYPOINT_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote waypoint runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `WAYPOINT_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Waypoint server did not become ready");
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "waypoint@nightly" }),
      "waypoint@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "waypoint@nightly" }),
      "waypoint@nightly",
    );
    assert.include(
              ...
              return {
                ...
                stderr: Stream.make(
                  new TextEncoder().encode(
                    "Remote Waypoint server did not stop within 2 seconds.\n",
                  ),
                ),
              };
            }
