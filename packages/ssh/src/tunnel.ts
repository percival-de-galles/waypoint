...
} from "@waypoint/shared/httpReadiness";
import * as NetService from "@waypoint/shared/Net";
import { extractJsonObject, fromLenientJson } from "@waypoint/shared/schemaJson";
import { satisfiesSemverRange } from "@waypoint/shared/semver";
...
const DEFAULT_REMOTE_PORT = 4773;
...
export interface RemoteWaypointRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteWaypointRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteWaypointRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  WAYPOINT_NODE_ENGINE_RANGE=@@WAYPOINT_NODE_ENGINE_RANGE@@
  if [ -z "$WAYPOINT_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$WAYPOINT_NODE_ENGINE_RANGE" <<'NODE'
@@WAYPOINT_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for WAYPOINT_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$WAYPOINT_NODE_BIN/node" ]; then
        PATH="$WAYPOINT_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@WAYPOINT_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
WAYPOINT_NODE_SCRIPT_PATH=@@WAYPOINT_NODE_SCRIPT_PATH@@
if [ -n "$WAYPOINT_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$WAYPOINT_NODE_SCRIPT_PATH" "$@"
fi
if command -v waypoint >/dev/null 2>&1; then
  exec waypoint "$@"
fi
if [ -z @@WAYPOINT_PACKAGE_SPEC@@ ]; then
  printf 'Waypoint package installation is not configured. Install your source build on the remote host, or set WAYPOINT_NPM_PACKAGE to a package you own on the desktop host.\\n' >&2
  exit 1
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (waypoint depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a waypoint executable. \`npx --yes\` then exits 0 without
...
require_installed_waypoint_cli() {
  if ! WAYPOINT_CLI_PATH="$("$@" -- sh -c 'command -v waypoint')"; then
    printf 'Remote host could not install %s. See npm output above for the cause.\\n' @@WAYPOINT_PACKAGE_SPEC@@ >&2
    return 1
  fi
  if [ -n "$WAYPOINT_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no waypoint executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@WAYPOINT_PACKAGE_SPEC@@ >&2
  return 1
}
# The launcher records this PID, so exec the CLI without an npm wrapper process.
if command -v npx >/dev/null 2>&1; then
  require_installed_waypoint_cli npx --yes --package @@WAYPOINT_PACKAGE_SPEC@@ || exit 1
  exec "$WAYPOINT_CLI_PATH" "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_waypoint_cli npm exec --yes --package @@WAYPOINT_PACKAGE_SPEC@@ || exit 1
  exec "$WAYPOINT_CLI_PATH" "$@"
fi
printf 'Remote host is missing the waypoint CLI and could not install @@WAYPOINT_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@WAYPOINT_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.waypoint/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.waypoint"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-waypoint.sh"
RUNNER_NEXT="$STATE_DIR/run-waypoint.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@WAYPOINT_DEFAULT_REMOTE_PORT@@" "@@WAYPOINT_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@WAYPOINT_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@WAYPOINT_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@WAYPOINT_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@WAYPOINT_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@WAYPOINT_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@WAYPOINT_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env WAYPOINT_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@WAYPOINT_READY_TIMEOUT_MS@@"; then
    printf 'Remote Waypoint server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.waypoint/ssh-launch/@@WAYPOINT_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.waypoint"
RUNNER_FILE="$STATE_DIR/run-waypoint.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote Waypoint server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
...
export function buildRemoteWaypointRunnerScript(input?: RemoteWaypointRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      WAYPOINT_PACKAGE_SPEC: packageSpec,
      WAYPOINT_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      WAYPOINT_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteWaypointRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      WAYPOINT_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      WAYPOINT_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteWaypointRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    WAYPOINT_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    WAYPOINT_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteWaypointRunnerScript(input)),
    WAYPOINT_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    WAYPOINT_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    WAYPOINT_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    WAYPOINT_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    WAYPOINT_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    WAYPOINT_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    WAYPOINT_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteWaypointRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    WAYPOINT_STATE_KEY: remoteStateKey(target),
    WAYPOINT_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteWaypointRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteWaypointRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteWaypointRunnerOptions;
...
>()("@waypoint/ssh/tunnel/SshEnvironmentManager") {
