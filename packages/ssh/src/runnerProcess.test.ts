...
import { buildRemoteStopScript, buildRemoteWaypointRunnerScript } from "./tunnel.ts";
...
server.listen(Number(process.env.WAYPOINT_TEST_PORT ?? 0), "127.0.0.1", () => {
...
fs.appendFileSync(process.env.WAYPOINT_TEST_CALLS, JSON.stringify(args) + "\\n");
if (args.includes("--package")) {
  process.stdout.write(process.env.WAYPOINT_TEST_CLI + "\\n");
} else {
  const child = childProcess.spawn(process.execPath, [process.env.WAYPOINT_TEST_CLI, ...args], { stdio: "inherit" });
            ...
            Effect.gen(function* () {
              const child = yield* spawner.spawn(
                ChildProcess.make("/bin/sh", ["-s", "--", "serve", "a path with spaces"], {
                  ...
                  env: {
                    PATH: bin,
                    WAYPOINT_TEST_CLI: cliPath,
                    WAYPOINT_TEST_CALLS: callsPath,
                    WAYPOINT_TEST_PORT: String(port),
                  },
                  detached: false,
                  stdin: Stream.make(
                    new TextEncoder().encode(buildRemoteWaypointRunnerScript({ packageSpec })),
                  ),
                }),
              );
    ...
    it.live.each(["graceful", "timeout", "external"] as const)(
      ...
      (mode) =>
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
          const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-stop-" });
...
`,
          ...
          const isolatedScript = script.replace(
            /^STATE_DIR=.*$/mu,
            'STATE_DIR="$WAYPOINT_TEST_STATE_DIR"',
          );
          ...
          const runStop = Effect.fn("test.remoteStop")(function* () {
            const stop = yield* spawner.spawn(
              ChildProcess.make("/bin/sh", ["-s"], {
                cwd: fixture,
                env: { WAYPOINT_TEST_STATE_DIR: fixture },
  ...
  () => {
    ...
    it.live.each(cases)("handles $packageManager/$mode", ({ packageManager, mode }) =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
        const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-runner-install-" });
        const bin = path.join(fixture, "bin");
        const cliPath = path.join(fixture, "installed cli.mjs");
        const callsPath = path.join(fixture, "installer-calls.jsonl");
        const packageSpec = "waypoint@0.0.39-nightly.20260905.1286";
...
`,
        ...
        yield* fs.writeFileString(
          ...
          `#!/usr/bin/env node
const fs = require("node:fs");
fs.appendFileSync(process.env.WAYPOINT_TEST_CALLS, JSON.stringify(process.argv.slice(2)) + "\\n");
const mode = process.env.WAYPOINT_TEST_MODE;
...
`,
        ...
        yield* fs.chmod(path.join(bin, packageManager), 0o700);
        if (mode === "existing-cli") yield* fs.symlink(cliPath, path.join(bin, "waypoint"));
        ...
        if (installFailed) {
          ...
          assert.include(stderr, "npm produced no waypoint executable");
