...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@waypoint/contracts";
...
const PUBLISHABLE_WAYPOINT_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteWaypointCliPackageSpec(input: {
  ...
  if (!input.isDevelopment && PUBLISHABLE_WAYPOINT_VERSION_PATTERN.test(appVersion)) {
  ...
  return `${packageName}@${input.updateChannel === "nightly" ? "nightly" : "latest"}`;
}
