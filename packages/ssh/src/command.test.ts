...
import {
  ...
  resolveRemoteWaypointCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote waypoint package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteWaypointCliPackageSpec({ appVersion: "0.0.17", updateChannel: "latest" }),
        "",
      );
      assert.equal(
        resolveRemoteWaypointCliPackageSpec({
          packageName: "@example/waypoint",
          appVersion: "0.0.17",
          updateChannel: "latest",
        }),
        "@example/waypoint@0.0.17",
      );
      assert.equal(
        resolveRemoteWaypointCliPackageSpec({
      ...
      assert.equal(
        ...
        "waypoint@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteWaypointCliPackageSpec({
          packageName: "waypoint",
