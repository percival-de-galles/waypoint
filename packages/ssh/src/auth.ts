...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@waypoint/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "waypoint-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Waypoint re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# WAYPOINT_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${WAYPOINT_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$WAYPOINT_SSH_AUTH_SECRET"
  exit 0
fi
printf 'Waypoint ssh-askpass invoked without WAYPOINT_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Waypoint re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if WAYPOINT_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:WAYPOINT_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:WAYPOINT_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("Waypoint ssh-askpass invoked without WAYPOINT_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "waypoint-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { WAYPOINT_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "waypoint" }),
  };
});
