...
}).annotate({ description: "Links an authenticated cloud user to a Waypoint environment." });
...
>()("@waypoint/contracts/relay/RelayClientPrincipal") {}
...
>()("@waypoint/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Waypoint Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["waypoint-mobile", "waypoint-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "waypoint-mobile" as const;
export const RelayWebClientId = "waypoint-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Waypoint Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Waypoint environments, connecting authorized clients, and publishing agent activity.",
  );
