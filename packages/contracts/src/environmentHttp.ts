...
>()("@waypoint/contracts/environmentHttp/EnvironmentAuthenticatedPrincipal") {}
...
class EnvironmentMetadataHttpApi extends HttpApiGroup.make("metadata").add(
  HttpApiEndpoint.get("descriptor", "/.well-known/waypoint/environment", {
...
class EnvironmentConnectHttpApi extends HttpApiGroup.make("connect")
  ...
  .add(
    HttpApiEndpoint.post("health", "/api/waypoint-connect/health", {
  ...
  .add(
    HttpApiEndpoint.post("waypointMintCredential", "/api/waypoint-connect/mint-credential", {
      payload: RelayCloudMintCredentialRequest,
      success: RelayEnvironmentMintResponse,
      error: EnvironmentHttpCloudErrors,
    }),
