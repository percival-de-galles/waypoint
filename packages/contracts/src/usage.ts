 ...
 *
 * Each environment scans provider-owned transcripts for Claude, Codex, and
 * Grok, and records completed Waypoint turns for SDK-backed providers such as
 * OpenCode and Pi. The former keeps coverage outside Waypoint; the latter has
 * no stable provider transcript to scan.
 *
 ...
 *
 * v6 adds OpenCode and Pi to {@link UsageProviderKind}; v4 Claude/Codex buckets
 * remain valid, so mixed-version environments keep those totals instead of
 * treating every older server as stale.
 */
export const USAGE_MERGE_COMPATIBLE_SINCE = 4 as const;

export const UsageProviderKind = Schema.Literals(["claude", "codex", "grok", "opencode", "piAgent"]);
