...
import { resolveGitWorktreePath, resolveWorktreeWaypointHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeWaypointHome", () => {
  it.effect("answers with .waypoint before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeWaypointHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".waypoint"));
