import type { ProjectId, ProjectScript, ServerSettings } from "@waypoint/contracts";
...
export function projectScriptRuntimeEnv(
  ...
  const env: Record<string, string> = {
    WAYPOINT_PROJECT_ROOT: input.project.cwd,
  };
  if (input.worktreePath) {
    env.WAYPOINT_WORKTREE_PATH = input.worktreePath;
  }
  if (input.extraEnv) {
