...
describe("connectAuth", () => {
  it("round-trips state and challenge through the authorize URL fragment", () => {
    const url = buildConnectAuthorizeRequestUrl({
      hostedAppUrl: "https://waypoint.invalid",
      state: "q7mK9xV2pL4nR8sT6wYzAQ",
      challenge: "E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM",
    });
    const parsed = new URL(url);

    expect(parsed.origin).toBe("https://waypoint.invalid");
  ...
  it("rejects authorize requests missing state or challenge", () => {
    expect(readConnectAuthorizeRequest(new URL("https://waypoint.invalid/connect"))).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://waypoint.invalid/connect#state=abc")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://waypoint.invalid/connect#challenge=abc")),
    ).toBeNull();
  });
  ...
  it("rejects authorize requests whose loopback port is corrupted", () => {
    for (const port of ["", "abc", "-1", "0", "65536", "34338x", "34 38"]) {
      const url = new URL(
        `https://waypoint.invalid/connect#state=state-1&challenge=challenge-1&port=${encodeURIComponent(port)}`,
      );
  ...
  it("builds a PKCE authorize URL against the Clerk endpoint", () => {
    const url = new URL(
      buildConnectClerkAuthorizeUrl({
        authorizationEndpoint: "https://clerk.waypoint.invalid/oauth/authorize",
        clientId: "oauthapp_123",
        redirectUri: connectCallbackUrl("https://waypoint.invalid"),
    ...
    expect(url.origin).toBe("https://clerk.waypoint.invalid");
    expect(url.pathname).toBe("/oauth/authorize");
    expect(url.searchParams.get("client_id")).toBe("oauthapp_123");
    expect(url.searchParams.get("redirect_uri")).toBe("https://waypoint.invalid/connect/callback");
