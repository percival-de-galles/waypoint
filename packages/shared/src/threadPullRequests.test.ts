...
function link(
  ...
  return {
    ...
    number,
    url: `https://github.com/pingdotgg/waypoint/pull/${number}`,
    source: "manual",
...
describe("threadPullRequestKeysEqual", () => {
  it("ignores host and repository case", () => {
    expect(
      threadPullRequestKeysEqual(
        { host: "GitHub.com", repository: "PingDotGG/waypoint", number: 1 },
        { host: "github.com", repository: "pingdotgg/waypoint", number: 1 },
      ),
    ).toBe(true);
    expect(
      threadPullRequestKeysEqual(
        { host: "github.com", repository: "pingdotgg/waypoint", number: 1 },
        { host: "gitlab.com", repository: "pingdotgg/waypoint", number: 1 },
      ),
    ).toBe(false);
  });
});
...
describe("resolveThreadCurrentPullRequest", () => {
  ...
  it("reports a stack when several links are open and puts the highest layer on top", () => {
    const stack = {
      ...
      id: "s1",
      number: 1,
      url: "https://github.com/pingdotgg/waypoint/stacks/1",
      base: "main",
      layers: [
...
describe("legacyLinkedPullRequestOf", () => {
  const identity = {
    ...
    provider: "github",
    displayName: "pingdotgg/waypoint",
    locator: {
      source: "git-remote" as const,
      remoteName: "origin",
      remoteUrl: "https://github.com/pingdotgg/waypoint.git",
    },
  };
  it("projects only links the owning project can route without a host", () => {
    expect(legacyLinkedPullRequestOf([link(7)], "project-1" as never, identity)).toEqual({
      ...
      repository: "pingdotgg/waypoint",
      number: 7,
      url: "https://github.com/pingdotgg/waypoint/pull/7",
    });
  ...
  it.each([
    {
      ...
      url: "https://github.enterprise.test/pingdotgg/waypoint/pull/7",
    },
...
describe("chain selection and badge state", () => {
  ...
  it("keeps branch matching case-sensitive while ignoring repository case", () => {
    const bottom = link(1, {
      repository: "PingDotGG/Waypointcode",
...
describe("threadPullRequestSearchTerms", () => {
  it("includes completed and unsynced links but excludes dismissed links", () => {
    ...
    expect(terms).toContain("pingdotgg/waypoint#12");
    expect(terms).toContain("https://github.com/pingdotgg/waypoint/pull/12");
...
it("searches the legacy projection when old environments decode to an empty links list", () => {
  const linkedPullRequest = {
    ...
    repository: "pingdotgg/waypoint",
    number: 12,
    url: "https://github.com/pingdotgg/waypoint/pull/12",
  };
