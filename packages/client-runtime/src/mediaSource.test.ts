...
describe("resolveMediaSource", () => {
  describe("direct URLs", () => {
    ...
    it.each(["data:image/png;base64,AAAA", "blob:https://waypoint.invalid/id"])(
      "loads %s directly",
      (href) => {
        expect(resolveMediaSource(href, { threadId, imageEmbed: true })).toMatchObject({
          access: "direct",
  ...
  it("serves Waypoint attachment files in place like any other host path", () => {
    const path = `/home/demo/.waypoint/userdata/attachments/${attachmentId}.mp4`;
