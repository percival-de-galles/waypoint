...
describe("advertised endpoint helpers", () => {
  it("normalizes HTTP and WebSocket base URLs", () => {
    ...
    expect(deriveWsBaseUrl("http://127.0.0.1:4773")).toBe("ws://127.0.0.1:4773/");
  });

  it("marks HTTP endpoints as blocked from hosted HTTPS apps", () => {
    expect(classifyHostedHttpsCompatibility("http://192.168.1.44:4773")).toBe(
  ...
  it("creates provider-neutral endpoint records", () => {
    expect(
      createAdvertisedEndpoint({
        id: "lan:http://192.168.1.44:4773",
        label: "LAN",
        provider: coreProvider,
        httpBaseUrl: "http://192.168.1.44:4773",
        reachability: "lan",
        source: "desktop-core",
        isDefault: true,
      }),
      ...
      provider: coreProvider,
      httpBaseUrl: "http://192.168.1.44:4773/",
      wsBaseUrl: "ws://192.168.1.44:4773/",
      reachability: "lan",
      compatibility: {
