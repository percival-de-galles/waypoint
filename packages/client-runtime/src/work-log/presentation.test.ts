...
import { ThreadId } from "@waypoint/contracts";
...
describe("resolveWorkEntryToolPresentation", () => {
  it.each([
    "mcp__waypoint__preview_click",
    "mcp__waypoint__preview_click",
    "mcp__waypoint__preview_click",
    "Waypoint-code.preview_click",
    "waypoint · preview_click completed",
    "waypoint/preview_click",
    "preview_click",
  ...
  it("labels device tools with the device icon", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__waypoint__device_open",
        toolLifecycleStatus: "completed",
      }),
    ).toEqual({ displayName: "Opened a device in the Device panel", icon: "device" });
    expect(resolveWorkEntryToolPresentation({ label: "waypoint · device_screenshot" })).toEqual({
  ...
  it("uses structured MCP identity when the provider supplies a custom title", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolData: { server: "waypoint", tool: "preview_snapshot", result: { title: "Example" } },
      }),
  ...
  it.each([
    ...
    expect(
      resolveWorkEntryToolPresentation({
        label: "Waypoint-code.preview_click",
        toolLifecycleStatus,
      }),
  ...
  it("uses the summary's state only when the provider omitted a lifecycle status", () => {
    const entry = { label: "Waypoint-code.preview_click" };
  ...
  it.each([
    ...
    ["waypoint_thread_read", "Reading a Waypoint thread", "Read a Waypoint thread"],
    ["waypoint_thread_send", "Sending to a Waypoint thread", "Sent to a Waypoint thread"],
    [
      "waypoint_worktree_handoff",
    ...
    const entry = { label: `waypoint.${tool}` };
    expect(
  ...
  it("keeps Waypoint branding for non-browser tools and falls back to the original tool label", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__waypoint__task_status",
        toolTitle: "Check the child task",
      }),
    ).toEqual({ displayName: "Getting delegated task status", icon: "waypoint" });
  });

  it("does not brand unknown tools or another server's matching tool name", () => {
    for (const label of [
      "mcp__github__preview_click",
      "waypoint.unknown_tool",
      "waypoint.toString",
      "Search files",
    ]) {
...
describe("browser group summaries", () => {
  const browserEntry: WorkLogPresentationEntry = {
    label: "MCP tool call",
    toolData: { server: "waypoint", tool: "preview_click" },
  ...
  it("preserves first-seen action ordering alongside non-browser tools", () => {
    expect(
      summarizeToolGroup([
        ...
        {
          ...browserEntry,
          toolData: { server: "waypoint", tool: "task_status" },
        },
      ]),
      ...
      summarizeToolGroup([
        {
          ...
          toolData: { toolName: "mcp__waypoint__preview_evaluate" },
        },
      ]),
...
describe("resolveViewedImageAsset", () => {
  ...
  it("serves waypoint attachment paths in place like any other host path", () => {
    const path = "/Users/demo/.waypoint/dev/attachments/11111111-1111-4111-8111-111111111111.png";
...
describe("pull request tool presentation", () => {
  it.each([
    "mcp__waypoint__link_pull_request",
    "mcp__waypoint__link_pull_request",
    "Waypoint-code · link_pull_request",
    "waypoint/link_pull_request",
    "link_pull_request",
  ...
  it("recognizes unlink targets supplied as repository and number", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolLifecycleStatus: "completed",
        toolData: {
          toolName: "mcp__waypoint__unlink_pull_request",
  ...
  it("summarizes native PR work separately from ordinary tools and integration metadata", () => {
    const link: WorkLogPresentationEntry = {
      label: "Waypoint-code · link_pull_request",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolLifecycleStatus: "completed",
      toolSource: { key: "waypoint", name: "Waypoint", kind: "integration" },
    };
    const list: WorkLogPresentationEntry = {
      ...link,
      label: "Waypoint-code · list_thread_pull_requests",
    };
    expect(summarizeToolGroup([link, link, list])).toBe(
      "Linked 2 pull requests and checked linked pull requests",
    );
    expect(summarizeToolGroup([{ ...link, label: "Waypoint-code · unlink_pull_request" }])).toBe(
...
describe("device group summaries", () => {
  ...
  it("recognizes Claude tool names and preserves screenshot previews", () => {
    const entry = {
      ...
      toolData: { toolName: "mcp__waypoint__device_screenshot" },
  ...
  it("does not classify another server's tools as Waypoint device controls", () => {
    expect(
      summarizeToolGroup([
        {
          ...deviceEntry("device_open"),
