...
import { classifyMarkdownImageSource } from "@waypoint/client-runtime/markdown-images";
import { resolveMediaSource } from "@waypoint/client-runtime/media-source";
import { parseChangeRequestUrl } from "@waypoint/shared/changeRequestUrl";
import { isWorkspaceImagePreviewPath } from "@waypoint/shared/filePreview";
...
const WAYPOINT_MCP_TOOL_LABELS: Record<
  string,
...
> = {
  ...
  create_threads: ["Create", "Creating", "Created", "Waypoint threads"],
  waypoint_thread_start: ["Start", "Starting", "Started", "a Waypoint thread"],
  waypoint_thread_list: ["List", "Listing", "Listed", "Waypoint threads"],
  waypoint_thread_read: ["Read", "Reading", "Read", "a Waypoint thread"],
  waypoint_thread_send: ["Send", "Sending", "Sent", "to a Waypoint thread"],
  waypoint_thread_wait: ["Wait", "Waiting", "Waited", "for a Waypoint thread"],
  waypoint_thread_interrupt: ["Interrupt", "Interrupting", "Interrupted", "a Waypoint thread"],
  waypoint_worktree_handoff: ["Hand off", "Handing off", "Handed off", "thread to a git worktree"],
  waypoint_worktree_status: ["Get", "Getting", "Got", "thread worktree status"],
...
function resolveWaypointMcpToolPresentation(
  ...
  const name = normalizeCompactToolLabel(value).replace(
    /^(?:mcp__(?:waypoint|waypoint|waypoint)__|(?:waypoint|waypoint|waypoint)(?:[.:/]|\s*·\s*))/i,
    "",
  );
  if (!Object.hasOwn(WAYPOINT_MCP_TOOL_LABELS, name)) return null;

  const [action, running, completed, detail] = WAYPOINT_MCP_TOOL_LABELS[name]!;
  const verb =
    status === "inProgress"
      ? running
      : status === "completed"
...
export function resolveWorkEntryToolPresentation(
  ...
  if (data !== null && typeof data === "object") {
    if (
      ...
      return resolveWaypointMcpToolPresentation(`${data.server}.${data.tool}`, status, data);
    }
    if ("toolName" in data && typeof data.toolName === "string") {
      return resolveWaypointMcpToolPresentation(data.toolName, status, data);
    }
  }

  return (
    resolveWaypointMcpToolPresentation(entry.toolTitle, status, data) ??
    resolveWaypointMcpToolPresentation(entry.label, status, data)
  );
}
