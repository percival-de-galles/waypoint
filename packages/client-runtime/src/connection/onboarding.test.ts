import { AuthStandardClientScopes, EnvironmentId } from "@waypoint/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/waypoint/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/waypoint/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("Waypoint Test");
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      const registration = yield* prepareBearerConnectionUpdate({
        input: {
          ...
          httpBaseUrl: "http://100.65.180.100:4773/path",
        },
        entry: Option.some({
      ...
      expect(registration).toMatchObject({
        ...
        profile: {
          environmentId,
          label: "Renamed environment",
          httpBaseUrl: "http://100.65.180.100:4773/",
          wsBaseUrl: "ws://100.65.180.100:4773/",
        },
