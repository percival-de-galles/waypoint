...
describe("isTransportConnectionErrorMessage", () => {
  ...
  it("returns true for the Waypoint server WebSocket message", () => {
    expect(isTransportConnectionErrorMessage("Unable to connect to the Waypoint server WebSocket.")).toBe(
      true,
    );
  });
