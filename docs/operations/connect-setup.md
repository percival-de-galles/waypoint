# Waypoint Connect setup

Deployment and client configuration for Waypoint Connect. The [architecture note](../internals/waypoint-connect.md)
explains the trust boundaries; the [relay README](../../infra/relay/README.md#deployment) owns relay
provisioning instructions.

## Public application configuration

Waypoint Connect is disabled in a fresh clone. To build against the production deployment, copy the
...
```dotenv
WAYPOINT_CLERK_PUBLISHABLE_KEY=<publishable key>
WAYPOINT_CLERK_JWT_TEMPLATE=<JWT template name>
WAYPOINT_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
WAYPOINT_RELAY_URL=https://relay.example.com
```
...
1. Create a public OAuth application for the Waypoint CLI, using authorization-code exchange with PKCE.
2. Allow both redirect URIs: `http://127.0.0.1:34338/callback` and
   `https://waypoint.invalid/connect/callback`. A custom `WAYPOINT_HOSTED_APP_URL` needs its own
...
4. Set `WAYPOINT_CLERK_CLI_OAUTH_CLIENT_ID` to the generated public client ID in local and release
   build environments.

## JWT template

Create a Clerk JWT template named `waypoint-relay` with claims:

```json
{ "aud": "waypoint-relay" }
```

Set `WAYPOINT_CLERK_JWT_TEMPLATE=waypoint-relay` for clients and
`CLERK_JWT_AUDIENCE=waypoint-relay` for the relay. The production relay deployment environment
...
Development uses `waypoint-dev://app`; production uses `waypoint://app`. Update the array with
...
| Variant     | Callback                                      |
| ----------- | --------------------------------------------- |
| Development | `clerk://dev.waypoint.app.dev.callback`     |
| Preview     | `clerk://dev.waypoint.app.preview.callback` |
| Production  | `clerk://dev.waypoint.app.callback`         |

Preserve existing entries. These callbacks are separate from the `waypoint-dev` / `waypoint-preview` / `waypoint` navigation schemes. A private development build using the production Clerk key still needs its development callback allowed by that instance's administrator; rebuilding the same package does not change the allowlist.

## Desktop passkeys

For a production macOS app with bundle ID `dev.waypoint.app`:
...
4. Check `https://<frontend-api>/.well-known/apple-app-site-association`. Its
   `webcredentials.apps` must include `<TEAM_ID>.dev.waypoint.app`.
...
WAYPOINT_MACOS_PROVISIONING_PROFILE=/absolute/path/to/waypoint.provisionprofile
# Override only when the RP domain differs from the Clerk Frontend API hostname.
WAYPOINT_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```
...
WAYPOINT_PORT=13773 \
  "/Applications/Waypoint (Alpha).app/Contents/MacOS/Waypoint (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Waypoint (Alpha).app"
codesign -d --entitlements :- "/Applications/Waypoint (Alpha).app"
```
