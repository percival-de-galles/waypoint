...
Flags go directly after the task name, for example `vp run dev --home-dir /tmp/waypoint-dev`.
Add `--browser` to open a browser automatically.

### State and ports

Linked worktrees default to their own `.waypoint/userdata`, even when `WAYPOINT_HOME` is set.
The main checkout defaults to `~/.waypoint/dev/userdata`. An explicit `--home-dir` wins in both cases.
Never run a development server against the live `~/.waypoint/userdata`.
...
but occupied ports can shift them. `WAYPOINT_PORT_OFFSET` or `WAYPOINT_DEV_INSTANCE` can select a
...
`WAYPOINT_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
