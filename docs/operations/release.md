...
> For maintainers. Using Waypoint? See [docs/user](../user/).
...
- Reads the shared production Waypoint Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `waypoint`) with OIDC trusted publishing from the same workflow file:
...
## Waypoint Connect relay deployment
...
```sh
vp run --filter waypoint-relay deploy -- --stage "$USER" --env-file .env.local
```
...
The job looks up the `waypoint-marketing` project using the existing `VERCEL_TOKEN`
...
- `WAYPOINT_WEB_ROUTER_URL`: defaults to `https://waypoint.invalid`.
- `WAYPOINT_WEB_LATEST_DOMAIN`: defaults to `latest.waypoint.invalid`.
- `WAYPOINT_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.waypoint.invalid`.

Required Vercel domains:

- `waypoint.invalid`: the router domain users open, updated by stable releases.
- `latest.waypoint.invalid`: channel alias updated by stable releases.
- `nightly.waypoint.invalid`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__waypoint/channel?channel=latest` or
`/__waypoint/channel?channel=nightly`; the router stores the
`waypoint_web_channel` cookie and rewrites future requests on `waypoint.invalid` to
...
through `/__waypoint/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `waypoint.invalid` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `waypoint`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `waypoint@<version>` package available on
...
For a release smoke test, confirm `npm view waypoint@<version> version` returns the expected version, then
...
verify that the update stops before restart and run `npx waypoint@<version> service update` once on the
...
- Repository slug source:
  - `WAYPOINT_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
and extracts that archive into `~/.waypoint/wsl-runtime/sha256-<archive-digest>` inside
...
script temporarily prepares the `waypoint` package, then runs `vp pm publish --filter waypoint ...` from the
repository root so workspace publish configuration is applied correctly.

Checklist:

1. Confirm npm org/user owns package `waypoint` (or rename package first if needed).
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `waypoint` with npm dist-tag
`latest`, creates a real GitHub Release, aliases the hosted app to `latest.waypoint.invalid` and
`waypoint.invalid`, and can commit a version bump to `main` in the finalize job. Do not push a test tag
...
2. Create an explicit App ID for `dev.waypoint.app` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [Waypoint Connect setup](./connect-setup.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.dev.waypoint.app` and includes
    Associated Domains.
