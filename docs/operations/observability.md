...
Waypoint has one server-side observability model:
...
stdout/stderr at `~/.waypoint/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.waypoint/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.waypoint/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.waypoint/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.waypoint/dev/logs/server.trace.ndjson`.
...
```bash
export WAYPOINT_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export WAYPOINT_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export WAYPOINT_OTLP_SERVICE_NAME=waypoint-local
```

Optional:

```bash
export WAYPOINT_TRACE_MIN_LEVEL=Info
export WAYPOINT_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `WAYPOINT_OTLP_*`.

macOS app bundle example:

```bash
WAYPOINT_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
WAYPOINT_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
WAYPOINT_OTLP_SERVICE_NAME=waypoint-desktop \
...
```bash
TRACE_FILE="${WAYPOINT_HOME:-$HOME/.waypoint}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.waypoint/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.waypoint/dev/logs/server.trace.ndjson"
```
...
- service name such as `waypoint-local`, `waypoint-dev`, or `waypoint-desktop`
...
- `waypoint_rpc_request_duration`
- `waypoint_orchestration_command_duration`
- `waypoint_orchestration_command_ack_duration`
- `waypoint_provider_turn_duration`
- `waypoint_git_command_duration`

Counters tell you volume and failure rate:

- `waypoint_rpc_requests_total`
- `waypoint_orchestration_commands_total`
- `waypoint_provider_turns_total`
- `waypoint_git_commands_total`
...
`waypoint_orchestration_command_ack_duration` measures:
...
1. Check `waypoint_orchestration_command_ack_duration` by `commandType`.
...
- `WAYPOINT_OTLP_TRACES_URL` was not set
...
- `WAYPOINT_TRACE_FILE`: override trace file path
- `WAYPOINT_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `WAYPOINT_TRACE_MAX_FILES`: rotated file count, default `10`
- `WAYPOINT_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `WAYPOINT_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `WAYPOINT_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `WAYPOINT_OTLP_TRACES_URL`: OTLP trace endpoint
- `WAYPOINT_OTLP_METRICS_URL`: OTLP metric endpoint
- `WAYPOINT_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `WAYPOINT_OTLP_SERVICE_NAME`: service name, default `waypoint-server`
