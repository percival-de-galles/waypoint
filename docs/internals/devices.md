...
Device panel work over Tailscale and Waypoint Connect, including when an SSH host runs the devices.
...
npm-installed at a pinned version into the Waypoint home after its matching Device
...
session arbitrary command execution on the host, so Waypoint does not. The
...
snapshot model agents need and stays current with its own releases. Waypoint prepends
