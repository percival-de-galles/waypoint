...
`<app-id>.SnapShot` on the session bus and exports `dev.waypoint.SnapShot.Capture`; the config
...
Source in `apps/desktop/gnome-extension`, UUID `snap-shot@waypoint.invalid`. GNOME only discovers a newly
...
The extension trusts callers that own `dev.waypoint.App.SnapShot` (or the `.Development`
...
inside the extension with coordinates relative to Waypoint's content area. Electron 44's restored-session
