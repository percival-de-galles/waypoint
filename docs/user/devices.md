...
`agent-device` command line, which Waypoint sets up for them.
...
Studio's Device Manager. Waypoint detects standard SDK locations; set
...
to the thread. Mobile clients show device activity in the thread timeline. Agents drive the device through the `agent-device` command line. Waypoint
...
local network, Tailscale, and Waypoint Connect. Live video needs a secure page
...
Waypoint checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its Waypoint helpers when reachable; simulators keep running.

Waypoint provides discovery, streaming, and control. Arrange app builds,
