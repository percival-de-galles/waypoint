...
Waypoint requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let Waypoint start OpenCode locally. A password in
provider settings applies to both that server and Waypoint's connection. With no
...
settings. Waypoint uses only that configured password for an external server; it
...
restart before Waypoint can see configuration changes.
