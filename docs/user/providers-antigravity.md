...
Waypoint runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Waypoint to confirm
...
hostname. Only that Waypoint sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Waypoint's separate Plan mode
...
Waypoint keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Waypoint also reads `.gemini/skills` and the
