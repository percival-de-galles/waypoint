...
default preserves explicit project overrides. Workspace preferences in `waypoint.json` take precedence
...
let Waypoint detect an icon again.
...
Waypoint only pulls when it can fast-forward and the checkout has no changed files, untracked files,
