...
Waypoint must start again on that machine;
...
```sh
npx waypoint@<client-version> service update
```
...
For a foreground server, the copied command is `npx waypoint@<client-version>`. Add
