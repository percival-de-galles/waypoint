...
Native account-limit bars come from the Codex app server, Claude's first-party subscription
API, and OpenCode Go's account usage endpoint. The Go endpoint is independent of an agent
SDK: Waypoint can show the same Go account allowance when Pi resolves an `opencode-go`
credential, or for a local OpenCode instance with `OPENCODE_API_KEY` in its environment.

Pi otherwise reports request and context usage, not an account-wide allowance or reset
schedule. Antigravity's ACP protocol and generic OpenCode SDK expose model context ceilings,
but not subscription quota windows, so Waypoint does not invent a limit bar for them.
