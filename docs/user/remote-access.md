...
Connect a phone, browser, or another desktop app to Waypoint running on a different
machine. That machine must stay running and reachable while you work.

## Waypoint Connect

Waypoint Connect makes an environment available to your other devices without setting
up router forwarding. In the desktop app on the host, open **Settings →
Connections**, sign in, and enable **Waypoint Connect** for that environment.

For a command-line host, run:

```bash
npx waypoint@latest connect
```

Follow the sign-in instructions. Setup offers a
[background service](./background-service.md); if you decline it, start the
server with `npx waypoint serve`. Saving your sign-in alone does not make the machine
reachable.

On your other device, sign in to the same Waypoint Connect account and choose the
...
Waypoint Connect renews access credentials when needed without disconnecting a healthy
...
```bash
npx waypoint serve --host <private-ip>
```
...
```bash
npx waypoint serve --tailscale-serve
```

For an already-running server:

```bash
npx waypoint pair --tailscale
```
...
`--tailscale-serve-port`. See `npx waypoint pair --help` for other pairing options.

### Hosted web app

[waypoint.invalid](https://waypoint.invalid) needs an HTTPS endpoint. It connects directly
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. Waypoint starts
...
the connection stops a server that Waypoint launched; a server that was already
...
management is available through `npx waypoint auth --help`.

A session with an open connection stays listed after its access credential
expires.

To remove an environment from Waypoint Connect, open your account menu's **Waypoint Connect**
page, or **Settings → Waypoint Connect** on mobile, and choose **Deregister**. This
revokes its cloud access and frees its host space even when the environment is
offline or has been wiped.

On a command-line host, `waypoint connect unlink` disables exposure while retaining
your login; `waypoint connect logout` also clears that login. Background-service
...
## Waypoint Connect troubleshooting

Run `waypoint connect status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `waypoint service status` and read the displayed log. If it disappears
when SSH closes, see [background-service troubleshooting](./background-service.md#troubleshooting).

| Error                                                     | Recovery                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart Waypoint on the host.                                                                         |
| `auth_invalid` or `invalid_bearer`                        | Run `waypoint connect login`. If credentials were revoked, run `waypoint connect logout`, then `waypoint connect` again. Restart the server after signing in. |
| Expired or invalid link proof                             | Check the host's date and time, update Waypoint, then restart it.                                                                            |
...
`systemctl --user restart waypoint.service` for the background service. For a
foreground server, stop it and run `waypoint serve` again with your usual options.
...
devices. For server version warnings, follow [Updating Waypoint](./updating.md).
