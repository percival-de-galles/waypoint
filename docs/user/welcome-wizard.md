...
Waypoint shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Waypoint directly from a
...
- **Waypoint Connect** connects computers that are signed in to your account. Run
  `npx waypoint connect` on each computer you want to add, then start Waypoint or run
  `npx waypoint serve` so the computer stays available.
- **Add a computer** connects directly to a server on your network or tailnet.
  Start the server with `npx waypoint serve`, then run `npx waypoint pair --tailscale` and
  paste the pairing link. You can also run `npx waypoint serve --host <address>` and
  use `npx waypoint pair` when the server is already reachable on your network.

Saved computers and computers discovered through Waypoint Connect are selected by
...
If Waypoint cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Waypoint cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Waypoint checks each selected computer for Claude Code and Codex. If an agent is
...
Waypoint finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Waypoint keeps the
...
30 days. You can continue those conversations in Waypoint.

Conversation import is best effort. Waypoint keeps the first user prompt and the
...
conversation so Waypoint does not remove user text. It reads one conversation at
