...
Shift keys together. Waypoint attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Waypoint is in front captures Waypoint itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Waypoint refuses shortcuts
...
terminal contents. If an app is slow to answer, Waypoint attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Waypoint SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Waypoint under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Waypoint shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Waypoint shows the exact binding it will
...
**Other Wayland desktops.** Waypoint uses the desktop's screenshot portal when it can capture the
