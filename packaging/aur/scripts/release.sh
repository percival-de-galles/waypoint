...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Waypoint-Code-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit waypoint-icon.png
git config user.name 'waypoint-ci'
git config user.email 'waypoint-ci@users.noreply.github.com'
git add -A
