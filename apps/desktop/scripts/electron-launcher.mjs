// This file mostly exists because we want dev mode to say "Waypoint (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "Waypoint (Dev)" : "Waypoint (Alpha)";
const APP_BUNDLE_ID = isDevelopment
  ? `dev.waypoint.app.dev.${devBundleIdSuffix || "local"}`
  : "dev.waypoint.app";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["waypoint-dev"] : ["waypoint"];
...
// oxlint-disable-next-line waypoint/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentEnvironmentScript(environment) {
  const envEntries = [
    ...
    ["WAYPOINT_PORT", environment.WAYPOINT_PORT],
    ["WAYPOINT_HOME", environment.WAYPOINT_HOME],
    ["WAYPOINT_COMMIT_HASH", environment.WAYPOINT_COMMIT_HASH],
    ["WAYPOINT_OTLP_TRACES_URL", environment.WAYPOINT_OTLP_TRACES_URL],
    ["WAYPOINT_OTLP_EXPORT_INTERVAL_MS", environment.WAYPOINT_OTLP_EXPORT_INTERVAL_MS],
    ["WAYPOINT_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
...
export function makeDevelopmentLauncherScript({
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --waypoint-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSScreenCaptureUsageDescription:
      "Waypoint captures the active window when you use the snapshot shortcut.",
    NSDocumentsFolderUsageDescription: "Waypoint reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Waypoint name.
