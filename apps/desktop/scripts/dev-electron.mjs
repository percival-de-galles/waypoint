...
const remoteDebuggingPort = process.env.WAYPOINT_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line waypoint/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.WAYPOINT_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.WAYPOINT_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--waypoint-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--waypoint-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
