...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Waypoint");
  expect(findCaptureDestination([window(99, "Waypoint"), target], 42, "Waypoint")).toBe(target);
  expect(findCaptureDestination([window(99, "Waypoint")], 42, "Waypoint")).toBeUndefined();
...
it("does not guess between ambiguous windows from the same process", () => {
  expect(findCaptureDestination([window(42, "Waypoint"), window(42, "Waypoint")], 42, "Waypoint")).toBeUndefined();
