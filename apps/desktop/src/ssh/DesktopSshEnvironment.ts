...
import * as SshAuth from "@waypoint/ssh/auth";
import { resolveSshTarget } from "@waypoint/ssh/command";
import { discoverSshHosts } from "@waypoint/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@waypoint/ssh/errors";
import * as SshTunnel from "@waypoint/ssh/tunnel";
...
>()("@waypoint/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteWaypointRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Waypoint window is not available for SSH authentication.";
      break;
