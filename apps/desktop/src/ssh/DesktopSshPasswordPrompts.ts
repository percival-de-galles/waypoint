import type { DesktopSshPasswordPromptRequest } from "@waypoint/contracts";
import { DesktopSshPasswordPromptResolutionInputSchema } from "@waypoint/contracts";
import type { SshPasswordRequest } from "@waypoint/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedError<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `Waypoint window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
...
>()("@waypoint/desktop/ssh/DesktopSshPasswordPrompts") {}
