...
import { ConnectionCatalogDocument } from "@waypoint/client-runtime/platform";
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@waypoint/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ WAYPOINT_HOME: baseDir })),
    ),
  );
...
const withStore = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "waypoint-desktop-connection-catalog-test-",
    });
