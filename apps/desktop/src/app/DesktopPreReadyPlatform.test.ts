...
describe("DesktopPreReadyPlatform", () => {
  ...
  it.effect("disables Vulkan for Wayland unless graphics features were supplied explicitly", () => {
        ...
        Effect.sync(() => {
          assert.isTrue(
            appendSwitchMock.mock.calls.some(
              ([name, value]) => name === "disable-features" && value === "Vulkan",
            ),
          );
          vi.unstubAllEnvs();
        }),
  ...
  for (const previousEntry of [undefined, 'Exec="/Applications/deleted-previous.AppImage" %U']) {
    it.effect(
      ...
      () => {
        ...
        getSwitchValueMock.mockReturnValue("");
        let desktopName = "waypoint.desktop";
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/dev.waypoint.App.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "dev.waypoint.App.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=Waypoint (Alpha)");
            assert.include(identity.desktopEntry ?? "", "MimeType=x-scheme-handler/waypoint;");
          }),
    ...
    () =>
      Effect.gen(function* () {
        class ClerkShaped extends Context.Service<ClerkShaped, { readonly ready: true }>()(
          "@waypoint/desktop/app/DesktopPreReadyPlatform.test/ClerkShaped",
        ) {}
