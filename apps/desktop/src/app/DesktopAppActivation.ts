...
import { HostProcessUserId } from "@waypoint/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Waypoint could not process the desktop app request."),
        );
      });
    });
...
>()("@waypoint/desktop/app/DesktopAppActivation") {}
