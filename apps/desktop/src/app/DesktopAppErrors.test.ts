...
describe("DesktopApp errors", () => {
  it("preserves unavailable backend port context", () => {
    ...
    assert.equal(
      error.message,
      "No desktop backend port is available on hosts 127.0.0.1, 0.0.0.0, :: between 4773 and 65535.",
    );
  });
  ...
  it("reports the required development port", () => {
    ...
    assert.equal(error.message, "WAYPOINT_PORT is required in desktop development.");
  });
});
