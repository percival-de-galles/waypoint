...
const AppPackageMetadata = Schema.Struct({
  waypointCommitHash: Schema.optional(Schema.String),
});
...
>()("@waypoint/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.waypointCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
