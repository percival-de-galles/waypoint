...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { WAYPOINT_HOME: "/home/user/.waypoint-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.waypoint-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        WAYPOINT_HOME: "/home/user/.waypoint-test",
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "waypoint-dev",
      linuxDesktopEntryName: "dev.waypoint.App.Development.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.waypoint/dev when WAYPOINT_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.waypoint/dev/desktop-settings.json");
  ...
  it("treats whitespace-only WAYPOINT_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        WAYPOINT_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
