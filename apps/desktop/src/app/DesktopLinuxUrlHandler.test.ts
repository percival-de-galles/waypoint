...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    linuxDesktopEntryName: "dev.waypoint.App.desktop",
    linuxWmClass: "waypoint",
    linuxApplicationsDir: "/home/alice/.local/share/applications",
    appImagePath: Option.some("/home/alice/Applications/Waypoint-Code.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      ...
      execTarget: '/home/al ice/Apps/Waypoint "100%" $HOME\\x.AppImage',
      scheme: "waypoint",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=Waypoint (Nightly)");
    ...
    assert.include(
      entry,
      'Exec="/home/al ice/Apps/Waypoint \\\\"100%%\\\\" \\\\$HOME\\\\\\\\x.AppImage" %U',
    );
    assert.include(entry, "NoDisplay=true");
    assert.notInclude(entry, "StartupWMClass=");
    assert.include(entry, "MimeType=x-scheme-handler/waypoint;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "waypoint",
      desktopEntryPath: "/home/alice/.local/share/applications/dev.waypoint.App.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the waypoint:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/dev.waypoint.App.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the waypoint:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        ...
        'Exec="/home/alice/Applications/Waypoint-Code.AppImage" %U',
      );
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/waypoint;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "dev.waypoint.App.desktop", "x-scheme-handler/waypoint"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      yield* runRegister(recorded, {
        existingEntry: DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
          displayName: "Waypoint (Alpha)",
          execTarget: "/home/alice/Applications/Waypoint-Code.AppImage",
          scheme: "waypoint",
        }),
      });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(unpackaged, {
        environment: {
          isPackaged: false,
          linuxDesktopEntryName: "dev.waypoint.App.Development.desktop",
        },
      });
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/dev.waypoint.App.Development.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/dev.waypoint.App.desktop",
        }),
      });
