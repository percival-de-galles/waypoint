...
>()("@waypoint/desktop/app/DesktopPreReadyPlatform/DesktopPreReadyElectronOptions") {}
  ...
  return yield* Effect.sync((): DesktopPreReadyElectronOptions["Service"] => {
    ...
    if (linux !== null) {
      const isWayland =
        ...
        process.env.WAYLAND_DISPLAY?.trim().length;
      if (
        isWayland &&
        !Electron.app.commandLine.hasSwitch("enable-features") &&
        !Electron.app.commandLine.hasSwitch("disable-features")
      ) {
        // Electron 44 warns that its Wayland platform is incompatible with Vulkan.
        // Keep the normal GPU renderer and only opt out of that backend.
        Electron.app.commandLine.appendSwitch("disable-features", "Vulkan");
      }
