...
import { DesktopSnapShotId } from "@waypoint/contracts";
...
vi.mock("electron", async (importOriginal) => ({
  ...
  session: {
    fromPartition: vi.fn(() => ({
      getUserAgent: vi.fn(() => "Mozilla/5.0 Electron/41.5.0 waypoint/1.2.3"),
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.mock(PreviewManager.PreviewManager)({
          ...
          isBrowserPartition: (partition) => partition.startsWith("persist:waypoint-preview-"),
          getBrowserPartition: () => Effect.succeed("persist:waypoint-preview-test"),
          reapplyZoom: () =>
            Effect.sync(() => {
  ...
  it.effect("shows native context menus for browser guests and sign-in popups", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* desktopWindow.handleBackendReady(new URL("http://127.0.0.1:4773"));
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "waypoint://app/",
        navigationUrl: "waypoint://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it.effect("does not open a development window until the backend is ready", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], ["waypoint-dev://app/"]);
  ...
  it.effect("recovers when the development renderer is temporarily unreachable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        didFailLoad({}, -9, "ERR_UNEXPECTED", "waypoint-dev://app/", true);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "waypoint-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "waypoint-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          const readyExit = yield* Effect.exit(
            desktopWindow.handleBackendReady(new URL("http://127.0.0.1:4773")),
          );
  ...
  it.effect("delivers capture completion and late errors without taking focus back", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(foreground, "Waypoint");
        foreground = "Explorer";
