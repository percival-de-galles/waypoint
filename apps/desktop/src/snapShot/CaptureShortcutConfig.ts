...
export class CaptureShortcutConfig {
  ...
  async apply(id: string, desktop: Target["desktop"]): Promise<DesktopCaptureConfigApplied> {
    ...
    const temporary = NodePath.join(directory, `.waypoint-capture-${NodeCrypto.randomUUID()}.tmp`);
    let staged = false;
    try {
      ...
      await unchanged();
      const backupPath = `${root.resolvedPath}.waypoint-capture-backup-${NodeCrypto.randomUUID()}`;
