...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "waypoint-hypr-test-"));
  paths = {
...
it.each([true, false])("uses the active config syntax and user bindings (Lua: %s)", async (lua) => {
  ...
  const result = await hyprlandCaptureShortcut("dev.waypoint.App", directory);
  expect(result.shortcutConfigPath).toBe(
    NodePath.join(config, lua ? "bindings.lua" : "bindings.conf"),
  );
  expect(result.shortcutBinding).toBe(
    lua
      ? 'hl.bind("CTRL + SHIFT + 2", hl.dsp.global("dev.waypoint.App:capture-window"))'
      : "bind = CTRL SHIFT, 2, global, dev.waypoint.App:capture-window",
  );
