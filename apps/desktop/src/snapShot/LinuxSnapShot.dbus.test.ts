...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "waypoint-private-dbus-"));
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.WaypointSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("dev.waypoint.App")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("dev.waypoint.App")).toBe("gnome-extension");
    expect(await extension.captureExtension("dev.waypoint.App")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["dev.waypoint.App.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("dev.waypoint.App")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("dev.waypoint.App", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("Waypoint");
    ...
    expect(activateTitle).toBe("Waypoint");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("dev.waypoint.App")).toBe("kde");
    ...
    const command = [
      ...
      "--session",
      "--dest",
      "dev.waypoint.App.NiriTest.SnapShot",
      "--object-path",
      "/com/waypointtools/SnapShot",
      "--method",
      "dev.waypoint.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "dev.waypoint.App.NiriTest.SnapShot",
        path: "/com/waypointtools/SnapShot",
        interface: "dev.waypoint.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
