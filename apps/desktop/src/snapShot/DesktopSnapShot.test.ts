...
it.effect.each(["win32", "darwin", "linux"] as const)(
  ...
  (platform) => {
    ...
    const waypoint = {
      id: 42,
      title: "Waypoint",
      appIdentifier: "dev.waypoint.App.desktop",
      owner: { name: "Waypoint", processId: 123 },
      bounds,
      png: Buffer.from([1, 2, 3]),
    };
    focusedWindowMock.mockReturnValue({
      getBounds: () => bounds,
      getTitle: () => waypoint.title,
      isDestroyed: () => false,
      isMinimized: () => false,
      isVisible: () => true,
      show: vi.fn(),
    ...
    macCaptureMock.mockReset().mockImplementation(async () => {
      images.push(waypoint.png);
      return { source: { name: waypoint.title }, png: waypoint.png };
    });
    ...
    linuxCaptureMock.mockResolvedValueOnce({
      ...
      window: {
        ...
        appName: waypoint.owner.name,
        appIdentifier: waypoint.appIdentifier,
        processId: waypoint.owner.processId,
        bounds,
      },
      feedback: {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const saved = yield* decodePendingMetadata(metadata);
        assert.equal(saved.source.windowTitle, waypoint.title);
        assert.equal(saved.source.appName, waypoint.owner.name);
        assert.equal(saved.source.accessibleText, `Window from process ${waypoint.owner.processId}`);
        assert.deepEqual(images, [waypoint.png]);
        assert.equal(prepareCaptureRevealMock.mock.calls.length, platform === "win32" ? 1 : 0);
        if (platform === "linux") {
          assert.equal(saved.source.appIdentifier, waypoint.appIdentifier);
          assert.deepEqual(activate.mock.calls, [[waypoint.title]]);
        }
      }),
    ).pipe(
      Effect.provide(
      ...
      Effect.gen(function* () {
        ...
        const warning = logs.find(
          (message) =>
            ...
            message[0] === "The compositor could not activate Waypoint after the snapshot",
        );
...
it.each([
  { names: ["⠙ waypoint"], expected: "Verified text" },
  { names: ["⠋ waypoint", "⠙ waypoint"], expected: undefined },
...
it.effect("flags revoked macOS permissions on read and re-registers once they return", () => {
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      assert.equal(
        revoked.message,
        "Allow Screen Recording in System Settings, then restart Waypoint.",
      );
...
for (const fails of [false, true]) {
  it.effect(`tests macOS capture without publishing it and cleans up, failure=${fails}`, () => {
    const active = {
      ...
      id: 42,
      title: "Setup",
      owner: { name: "Waypoint", processId: 123, path: "/Applications/Waypoint.app" },
      bounds: { x: 0, y: 0, width: 800, height: 600 },
    };
    activeWindowMock.mockReset().mockResolvedValue(active);
