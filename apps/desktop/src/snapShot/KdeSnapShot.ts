...
import type { DesktopCaptureHelperState } from "@waypoint/contracts";
...
export const KDE_CAPTURE_EXECUTABLE = "waypoint-kde-snap-shot";
const DESKTOP_FILE = "dev.waypoint.App.KdeCapture.desktop";
const MARKER = "X-Waypoint-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "waypoint", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall Waypoint.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall Waypoint.",
        );
...
export async function captureKdeWindow(
  ...
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "waypoint-kde-capture-"));
  let retained = false;
      ...
      feedback: {
        ...
        activate: async (title) => {
          ...
          const activation = await NodeFSP.mkdtemp(
            NodePath.join(NodeOS.tmpdir(), "waypoint-kde-activate-"),
          );
          try {
            await run(
              executable,
