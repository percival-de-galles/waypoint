...
describe("findAccessibleWindow", () => {
  ...
  it.each(["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"])(
    ...
    (frame) => {
      const windows = [{ name: `${frame} waypoint`, bounds: captured.bounds }];

      expect(findAccessibleWindow(windows, { ...captured, title: "⠋ waypoint" }, "wayland")).toBe(
        windows[0],
      );
    },
  );
  ...
  it("rejects matching spinners when the window sizes differ", () => {
    expect(
      findAccessibleWindow(
        [{ name: "⠙ waypoint", bounds: { ...captured.bounds, width: 400 } }],
        { ...captured, title: "⠋ waypoint" },
        "wayland",
      ),
    ).toBeUndefined();
  });
  ...
  it("rejects ambiguous normalized titles even if one matches the captured spinner exactly", () => {
    const windows = [
      { name: "⠋ waypoint", bounds: captured.bounds },
      { name: "⠙ waypoint", bounds: captured.bounds },
    ];

    expect(
      findAccessibleWindow(windows, { ...captured, title: "⠋ waypoint" }, "wayland"),
    ).toBeUndefined();
  });

  it("keeps exact title matching outside Wayland", () => {
    expect(
      findAccessibleWindow([{ name: "⠙ waypoint", bounds: captured.bounds }], {
        ...captured,
        title: "⠋ waypoint",
      }),
    ).toBeUndefined();
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          const runtimeDirectory = await mkdtemp(join(tmpdir(), "waypoint-wayland-"));
          ...
          try {
            ...
            expect(isWaylandSession("linux", { XDG_RUNTIME_DIR: "/nonexistent-waypoint-test" })).toBe(
              false,
            );
          } finally {
            if (server.listening) {
