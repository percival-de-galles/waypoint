...
import { resolveRemoteWaypointCliPackageSpec } from "@waypoint/ssh/command";
import type { RemoteWaypointRunnerOptions } from "@waypoint/ssh/tunnel";
...
const resolveDesktopSshCliRunner = (
  ...
  packageName: string,
): RemoteWaypointRunnerOptions => {
  const devRemoteEntryPath = Option.getOrUndefined(environment.devRemoteWaypointServerEntryPath);
  ...
  return {
    packageSpec: resolveRemoteWaypointCliPackageSpec({
      packageName,
...
const desktopSshEnvironmentLayer = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const packageName = yield* Config.string("WAYPOINT_NPM_PACKAGE").pipe(Config.withDefault(""));
    return DesktopSshEnvironment.layer({
      resolveCliRunner: settings.get.pipe(
        Effect.map((currentSettings) =>
          resolveDesktopSshCliRunner(environment, currentSettings, packageName),
        ),
      ),
    });
  }),
