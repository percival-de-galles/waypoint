...
const withImporter = Effect.fnUntraced(function* () {
  ...
  const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-import-" });
  ...
  const importer = yield* BrowserImport.BrowserImport.pipe(
    Effect.provide(
      BrowserImport.layer.pipe(
        ...
        Layer.provide(Layer.succeed(HostProcessExecutablePath, "/Applications/Waypoint.app")),
    ...
    Effect.gen(function* () {
      ...
      const error = yield* importer
        .importCookies({
          ...
          scope: "persist:waypoint-preview-test",
          persistent: true,
        })
        .pipe(Effect.flip);
