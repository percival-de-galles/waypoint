...
const OVERLAY_ATTRIBUTE = "data-waypoint-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--waypoint-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--waypoint-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--waypoint-radius": theme.radius,
    "--waypoint-background": theme.background,
    "--waypoint-foreground": theme.foreground,
    "--waypoint-popover": theme.popover,
    "--waypoint-popover-foreground": theme.popoverForeground,
    "--waypoint-primary": theme.primary,
    "--waypoint-primary-foreground": theme.primaryForeground,
    "--waypoint-muted": theme.muted,
    "--waypoint-muted-foreground": theme.mutedForeground,
    "--waypoint-accent": theme.accent,
    "--waypoint-accent-foreground": theme.accentForeground,
    "--waypoint-border": theme.border,
    "--waypoint-input": theme.input,
    "--waypoint-ring": theme.ring,
    "--waypoint-font-sans": theme.fontSans,
    "--waypoint-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-waypoint-annotation-tool] body, html[data-waypoint-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-waypoint-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--waypoint-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-waypoint-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
