  ...
  it.effect("preserves the readiness timeout cause and process context", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.equal(request.url, "http://127.0.0.1:4773/.well-known/waypoint/environment");
        ...
        assert.equal(error.httpBaseUrl.href, "http://127.0.0.1:4773/");
        assert.equal(error.readinessUrl.href, "http://127.0.0.1:4773/.well-known/waypoint/environment");
        assert.equal(error.timeoutMs, 50);
        assert.isDefined(error.cause);
        assert.equal(
          error.message,
          "Timed out after 50ms waiting for desktop backend readiness at http://127.0.0.1:4773/.well-known/waypoint/environment.",
        );
  ...
  it.effect("retries HTTP readiness before reporting the backend ready", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.deepEqual(requestUrls, ["http://127.0.0.1:4773/.well-known/waypoint/environment"]);
        ...
        assert.deepEqual(requestUrls, [
          "http://127.0.0.1:4773/.well-known/waypoint/environment",
          "http://127.0.0.1:4773/.well-known/waypoint/environment",
        ]);
      }).pipe(Effect.provide(TestClock.layer())),
    ),
