...
>()("@waypoint/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "WAYPOINT_NO_BROWSER",
  "WAYPOINT_HOST",
  "WAYPOINT_DESKTOP_WS_URL",
  "WAYPOINT_DESKTOP_LAN_ACCESS",
  "WAYPOINT_DESKTOP_LAN_HOST",
  "WAYPOINT_DESKTOP_HTTPS_ENDPOINTS",
  "WAYPOINT_TAILSCALE_SERVE",
  "WAYPOINT_TAILSCALE_SERVE_PORT",
] as const;
...
function resourceMonitorBinaryName(platform: NodeJS.Platform): string {
  return platform === "win32" ? "waypoint-resource-monitor.exe" : "waypoint-resource-monitor";
}
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      waypointHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's WAYPOINT_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit waypointHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus WAYPOINT_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.waypoint via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutWaypointHome: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "WAYPOINT_HOME") continue;
    parentEnvWithoutWaypointHome[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutWaypointHome.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutWaypointHome,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus WAYPOINT_HOME; pass it
