...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ WAYPOINT_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { WAYPOINT_HOME: input.baseDir, ...input.env };
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "waypoint-desktop-server-exposure-test-",
    });
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("preserves explicit Tailscale exposure overrides", () =>
    withHarness(
      ...
      {
        WAYPOINT_DESKTOP_LAN_HOST: "100.90.1.2",
        WAYPOINT_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
  ...
  it.effect("advertises loopback, LAN, and configured manual endpoints from runtime state", () =>
    withHarness(
      ...
      Effect.gen(function* () {
        const serverExposure = yield* DesktopServerExposure.DesktopServerExposure;
        yield* serverExposure.configureFromSettings({ port: 4773 });
        yield* serverExposure.setMode("network-accessible");

        const endpoints = yield* serverExposure.getAdvertisedEndpoints;
        assert.deepEqual(endpoints, [
          {
            id: "desktop-loopback:4773",
            label: "This machine",
            provider: {
              id: "desktop-core",
              label: "Desktop",
          ...
          {
            id: "desktop-lan:http://192.168.1.20:4773",
            label: "Local network",
            provider: {
              id: "desktop-core",
              label: "Desktop",
            ...
            httpBaseUrl: "http://192.168.1.20:4773/",
            wsBaseUrl: "ws://192.168.1.20:4773/",
            reachability: "lan",
            compatibility: {
              hostedHttpsApp: "mixed-content-blocked",
              desktopApp: "compatible",
          ...
          {
            id: "manual:http://desktop.example.test:4773",
            ...
            httpBaseUrl: "http://desktop.example.test:4773/",
            wsBaseUrl: "ws://desktop.example.test:4773/",
            reachability: "public",
            compatibility: {
              hostedHttpsApp: "mixed-content-blocked",
              desktopApp: "compatible",
      ...
      }),
      {
        WAYPOINT_DESKTOP_HTTPS_ENDPOINTS:
          "https://desktop.example.ts.net,http://desktop.example.test:4773,not-a-url",
      },
    ),
  );
});
