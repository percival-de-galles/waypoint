...
describe("ElectronWindow", () => {
    ...
    Effect.gen(function* () {
      ...
      const options = {
        ...
        webPreferences: {
          ...
          partition: "persist:waypoint-preview-test",
          sandbox: true,
      ...
      assert.equal(error.message, 'Failed to create Electron BrowserWindow "Waypoint" (1100x780).');
  ...
  it.effect("focuses the exact Waypoint window before activating from a shell-hosted app", () =>
    Effect.gen(function* () {
      const operations: Array<string> = [];
  ...
  it.effect("prepares the exact Waypoint window before a capture overlay", () =>
    Effect.gen(function* () {
