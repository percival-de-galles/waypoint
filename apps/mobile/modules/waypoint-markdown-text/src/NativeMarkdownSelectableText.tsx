...
import { installMarkdownCopySanitizer } from "./WaypointMarkdownTextSelectionModule";
...
export function NativeMarkdownSelectableText(props: {
  ...
  // WaypointMarkdownText only rebuilds its attributed string during native layout. A
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              Platform.OS === "ios"
                ? run.fileIcon
                  ? `waypoint-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                  : run.skillName
                    ? "waypoint-skill:sf:cube"
                    : linkIcon
                      ? `waypoint-link:${Image.resolveAssetSource(markdownLinkIconSource(linkIcon)).uri}`
                      : undefined
                : undefined
            }
