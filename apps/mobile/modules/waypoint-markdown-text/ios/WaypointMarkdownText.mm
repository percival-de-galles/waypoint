...
#import "WaypointMarkdownTextShadowNode.h"
#import "WaypointMarkdownTextComponentDescriptor.h"
...
static void WaypointMarkdownTextApplyParagraphStyles(
...
static void WaypointMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
@protocol WaypointMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface WaypointMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<WaypointMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<WaypointMarkdownOutsideTapTarget>)target;

@end

static const void *WaypointMarkdownOutsideTapCoordinatorKey =
    &WaypointMarkdownOutsideTapCoordinatorKey;

@implementation WaypointMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<WaypointMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<WaypointMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, WaypointMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<WaypointMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static WaypointMarkdownOutsideTapCoordinator *
WaypointMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  WaypointMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, WaypointMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[WaypointMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        WaypointMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface WaypointMarkdownText () <RCTWaypointMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface WaypointMarkdownText () <WaypointMarkdownOutsideTapTarget>
@end

@implementation WaypointMarkdownText {
  UIView * _view;
  UITextView * _textView;
  WaypointMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<WaypointMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const WaypointMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    WaypointMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, WaypointMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [WaypointMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<WaypointMarkdownTextProps const>(_props);
  ...
  WaypointMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  WaypointMarkdownTextApplyAttachments(
      convertedAttrString,
      _state->getData().attachmentRanges,
      _attachmentImages);
  NSUInteger runLocation = 0;
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[WaypointMarkdownTextRun class]]) {
      continue;
    }

    WaypointMarkdownTextRun *textChild = (WaypointMarkdownTextRun *)child;
    ...
    NSURL *link = [NSURL URLWithString:
        [NSString stringWithFormat:@"waypoint-markdown-run://%ld", (long)textChild.tag]];
    if (link != nil) {
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::WaypointMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::WaypointMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<WaypointMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
...
{
  const auto &oldViewProps = *std::static_pointer_cast<WaypointMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<WaypointMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == WaypointMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == WaypointMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == WaypointMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == WaypointMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const WaypointMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable WaypointMarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (WaypointMarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  WaypointMarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu]) {
    return defaultAction;
  }

  __weak WaypointMarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::WaypointMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::WaypointMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> WaypointMarkdownTextCls(void)
{
  return WaypointMarkdownText.class;
}
