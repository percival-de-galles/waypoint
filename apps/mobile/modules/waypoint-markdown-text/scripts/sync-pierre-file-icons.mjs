...
const customSprite = webIconSource.match(/const WAYPOINT_FILE_ICON_SPRITE = `([\s\S]*?)`;/)?.[1];

if (!customSprite) {
  throw new Error("Could not read the Waypoint Pierre icon sprite from apps/web/src/pierre-icons.ts");
}
...
const customIcons = {
  agents: "waypoint-file-icon-agents",
  claude: "waypoint-file-icon-claude",
  package: "waypoint-file-icon-package-json",
  pnpm: "waypoint-file-icon-pnpm",
  readme: "waypoint-file-icon-readme",
  tsconfig: "waypoint-file-icon-tsconfig",
  video: "waypoint-file-icon-video",
};
