...
public final class WaypointKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("WaypointKeyboardCommands")

    View(WaypointKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: WaypointKeyboardCommandsView, commands: [String]) in
...
public final class WaypointKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.waypointFirstResponder == nil else { return }
...
private extension UIView {
  var waypointFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.waypointFirstResponder { return responder }
    }
    return nil
  }
}
