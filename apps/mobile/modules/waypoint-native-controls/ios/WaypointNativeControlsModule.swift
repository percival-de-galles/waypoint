...
public final class WaypointNativeControlsModule: Module {
  private let presentationSources = WaypointPresentationSources()
  private var videoPresentation: WaypointNativeVideoPresentation?
  private var filePresentation: WaypointNativeFilePresentation?

  public func definition() -> ModuleDefinition {
    Name("WaypointNativeControls")
    ...
    View(WaypointPresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: WaypointPresentationSourceView, identifier: String) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/WaypointShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("WaypointNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/WaypointShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "WaypointNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = WaypointNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func presentFile(url: URL, title: String, sourceIdentifier: String,
    ...
    let file = WaypointNativeFilePresentation(identifier: identifier, sources: presentationSources,
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "WaypointNativePresentation",
        code: 2,
