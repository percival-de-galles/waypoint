# Waypoint Mobile Terminal Native Module
...
custom-I/O and live-padding branch. `WaypointTerminalView` owns a `libghostty` surface and uses that
callback I/O model:
...
`waypoint/custom-io-ordered-feed` branch when vendored, based on VVTerm's
`vvterm/custom-io-padding` branch).

```bash
apps/mobile/modules/waypoint-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/waypoint-terminal/scripts/build-libghostty-android.sh
```
