...
> [!WARNING]
> Waypoint Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `Waypoint Dev`
- `preview`: persistent internal preview build, installable side-by-side as `Waypoint Preview`
- `production`: store/release build as `Waypoint`

Run commands from `apps/mobile`.

Waypoint Connect is optional and disabled in a fresh clone. Public configuration belongs in the
...
```bash
WAYPOINT_IOS_PERSONAL_TEAM=1 \
WAYPOINT_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.waypoint.dev \
vp run ios:dev
```
...
```bash
WAYPOINT_IOS_PERSONAL_TEAM=1 \
WAYPOINT_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.waypoint \
vp run ios:release
```
...
For preview or production EAS environments, set `WAYPOINT_CLERK_PUBLISHABLE_KEY`,
`WAYPOINT_CLERK_JWT_TEMPLATE`, and `WAYPOINT_RELAY_URL`
