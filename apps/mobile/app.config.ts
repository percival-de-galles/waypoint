...
const isIosPersonalTeamBuild = repoEnv.WAYPOINT_IOS_PERSONAL_TEAM === "1";
const runtimeVersionPolicy =
  process.env.MOBILE_VERSION_POLICY ??
  (APP_VARIANT === "development" ? "appVersion" : "fingerprint");
const mobileUpdateUrl = repoEnv.WAYPOINT_MOBILE_UPDATE_URL?.trim();
const mobileAppleTeamId = repoEnv.WAYPOINT_APPLE_TEAM_ID?.trim();
const mobileRelyingParty = repoEnv.WAYPOINT_CLERK_RELYING_PARTY?.trim() || "waypoint.invalid";
const easProjectId = repoEnv.WAYPOINT_EAS_PROJECT_ID?.trim();
const easOwner = repoEnv.WAYPOINT_EAS_OWNER?.trim();

const personalTeamBundleIdentifier = repoEnv.WAYPOINT_IOS_PERSONAL_TEAM_BUNDLE_ID?.trim();
...
if (
  ...
  throw new Error(
    "WAYPOINT_IOS_PERSONAL_TEAM_BUNDLE_ID must be a reverse-DNS identifier such as com.example.waypoint when WAYPOINT_IOS_PERSONAL_TEAM=1.",
  );
}
...
const VARIANT_CONFIG = {
  development: {
    appName: "Waypoint Dev",
    scheme: "waypoint-dev",
    iosBundleIdentifier: "dev.waypoint.app.dev",
    androidPackage: "dev.waypoint.app.dev",
    relyingParty: mobileRelyingParty,
    assets: DEVELOPMENT_ASSETS,
  },
  preview: {
    appName: "Waypoint Preview",
    scheme: "waypoint-preview",
    iosBundleIdentifier: "dev.waypoint.app.preview",
    androidPackage: "dev.waypoint.app.preview",
    relyingParty: mobileRelyingParty,
  ...
  production: {
    appName: "Waypoint",
    scheme: "waypoint",
    iosBundleIdentifier: "dev.waypoint.app",
    androidPackage: "dev.waypoint.app",
    relyingParty: mobileRelyingParty,
    ...
    widgets: [
      {
        ...
        description: "Shows the current state of active Waypoint agents.",
...
const config: ExpoConfig = {
  ...
  updates: mobileUpdateUrl
    ? {
        enabled: repoEnv.WAYPOINT_MOBILE_UPDATES_ENABLED !== "0",
        url: mobileUpdateUrl,
  ...
  ios: {
    ...
    requireFullScreen: process.env.WAYPOINT_SHOWCASE_CAPTURE_BUILD === "1",
    bundleIdentifier: iosBundleIdentifier,
    ...(mobileAppleTeamId ? { appleTeamId: mobileAppleTeamId } : {}),
    associatedDomains: [
    ...
    infoPlist: {
      ...
      NSLocalNetworkUsageDescription:
        "Allow Waypoint to connect to Waypoint servers on your local network or tailnet.",
      NSPhotoLibraryAddUsageDescription: "Allow Waypoint to save images to your photo library.",
      ...
      ...(process.env.WAYPOINT_SHOWCASE_CAPTURE_BUILD === "1"
        ? {
  ...
  android: {
    ...
    ...(repoEnv.WAYPOINT_ANDROID_GOOGLE_SERVICES_FILE
      ? { googleServicesFile: repoEnv.WAYPOINT_ANDROID_GOOGLE_SERVICES_FILE }
      : {}),
    adaptiveIcon: {
  ...
  plugins: [
    ...
    [
      "expo-audio",
      {
        microphonePermission: "Allow Waypoint to use your microphone for voice input.",
    ...
    [
      "expo-camera",
      {
        cameraPermission: "Allow Waypoint to access your camera so you can scan pairing QR codes.",
  ...
  extra: {
    ...
    relay: {
      url: repoEnv.WAYPOINT_RELAY_URL ?? null,
    },
    clerk: {
