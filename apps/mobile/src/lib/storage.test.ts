...
describe("mobile connection storage", () => {
  ...
  it("preserves secure-storage read failures with operation and key context", async () => {
    ...
    await expect(loadSavedConnections()).rejects.toMatchObject({
      ...
      operation: "read",
      key: "waypoint.connections",
      cause,
      message: "Mobile secure storage operation read failed for key waypoint.connections.",
    });
  });

  it("logs structured decode failures before using the empty fallback", async () => {
    await mocks.setItemAsync("waypoint.connections", "{");
    ...
    expect(warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message: "Failed to decode mobile storage value for key waypoint.connections.",
      }),
    );
  ...
  it("loads legacy preferences when SQLite is unavailable", async () => {
    ...
    await mocks.setItemAsync("waypoint.preferences", JSON.stringify({ baseFontSize: 17 }));
  ...
  it("persists Material You independently for each appearance", async () => {
    ...
    await savePreferencesPatch({ lightThemeId: "waypoint-chat" });
    await expect(loadPreferences()).resolves.toEqual({ ...themes, lightThemeId: "waypoint-chat" });
  });
  ...
  it("falls back to secure storage when SQLite cannot save preferences", async () => {
    ...
    const fallback = JSON.parse(mocks.getStoredValue("waypoint.preferences.fallback") ?? "") as {
  ...
  it("reconciles fallback preferences after SQLite recovers", async () => {
    ...
    expect(mocks.getStoredValue("waypoint.preferences.fallback")).toBeNull();
  });
  ...
  it("keeps SQLite authoritative when stale legacy preferences remain", async () => {
    mocks.setPreferencesJson(JSON.stringify({ baseFontSize: 21 }), 30);
    await mocks.setItemAsync("waypoint.preferences", JSON.stringify({ baseFontSize: 19 }));
