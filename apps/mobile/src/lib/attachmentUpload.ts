...
import { runAtomCommand, squashAtomCommandFailure } from "@waypoint/client-runtime/state/runtime";
import type {
  ChatFileAttachment,
  ChatImageAttachment,
  EnvironmentId,
...
import { PROVIDER_SEND_TURN_SUPPORTED_IMAGE_MIME_TYPES } from "@waypoint/contracts";
...
async function uploadFileBytes(
  ...
  const file =
    fileUri === undefined
      ? new File(Paths.cache, `waypoint-upload-${uuidv4()}`)
