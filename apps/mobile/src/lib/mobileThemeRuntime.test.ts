...
describe("mobileThemeRuntime", () => {
  it("keeps the default palette on Uniwind's built-in appearance themes", () => {
    expect(getMobileUniwindThemeName("waypoint", "light")).toBe("light");
    expect(getMobileUniwindThemeName("waypoint", "dark")).toBe("dark");
  });

  it("maps custom palettes and appearances to registered themes", () => {
    expect(getMobileUniwindThemeName("waypoint-chat", "dark")).toBe("waypoint-chat-dark");
  });
