...
} from "@waypoint/shared/threadEnvMode";
...
import { EnvironmentProject } from "@waypoint/client-runtime/state/shell";
import { type VcsRef } from "@waypoint/client-runtime/state/vcs";
import {
...
export function NewTaskFlowProvider(props: React.PropsWithChildren) {
  ...
  // checked-in waypoint.json, then the server's configured default.
  const waypointProjectFileQuery = useEnvironmentQuery(
    selectedProject !== null && selectedProject.workspaceRoot !== ""
      ? projectEnvironment.readFile({
          environmentId: selectedProject.environmentId,
          input: { cwd: selectedProject.workspaceRoot, relativePath: WAYPOINT_PROJECT_FILE_NAME },
        })
      : null,
  );
  const waypointProjectFileData = waypointProjectFileQuery.data as ProjectReadFileResult | null;
  const waypointProjectFileDefaultMode = useMemo(() => {
    if (waypointProjectFileData === null || waypointProjectFileData.truncated) return null;
    return parseWaypointProjectFile(waypointProjectFileData.contents)?.defaultThreadEnvMode ?? null;
  }, [waypointProjectFileData]);
  const defaultWorkspaceMode: WorkspaceMode = resolveDefaultThreadEnvMode({
    projectSetting: selectedProject?.defaultThreadEnvMode,
    projectFile: waypointProjectFileDefaultMode,
  ...
  // the frozen interim value beats the waypoint.json default once it loads.
  const defaultWorkspaceModeSettled = isDefaultThreadEnvModeSettled({
    explicitMode: selectedProjectDraft.workspaceSelection?.mode,
    projectSetting: selectedProject?.defaultThreadEnvMode,
    projectFilePending: waypointProjectFileQuery.isPending,
  });
