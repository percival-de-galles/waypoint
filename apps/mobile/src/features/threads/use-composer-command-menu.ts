import type { EnvironmentId, ProviderInteractionMode, ServerProvider } from "@waypoint/contracts";
...
} from "@waypoint/shared/composerTrigger";
import {
  insertRankedSearchResult,
  normalizeSearchQuery,
  scoreQueryMatch,
} from "@waypoint/shared/searchRanking";
import {
...
export function buildComposerSlashCommandItems(input: {
  ...
  /** Whether Waypoint itself offers /usage-limits for the selected provider. */
  ...
  // Providers expand commands only at the start of a message. Waypoint commands
  ...
  for (const command of input.selectedProviderStatus?.slashCommands ?? []) {
    ...
    // Waypoint's own limits command is answered by the thread composer; New Task has
