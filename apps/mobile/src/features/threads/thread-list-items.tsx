...
import type { EnvironmentThreadSearchMatch } from "@waypoint/client-runtime/state/thread-search";
import type { EnvironmentMachineKind } from "@waypoint/contracts";
    ...
    >
      ...
      >
        ...
        <Text
          className={
            compact
              ? "flex-shrink text-base font-waypoint-bold tracking-[0.2px] text-foreground-muted"
              : "flex-shrink text-sm font-waypoint-bold tracking-[0.2px] text-foreground-muted"
          }
          numberOfLines={1}
        >
          {props.title}
        ...
        <Text
          className={
            compact
              ? "flex-1 text-sm font-waypoint-medium text-foreground-tertiary"
              : "flex-1 text-xs font-waypoint-medium text-foreground-tertiary"
          }
        >
          {props.threadCount}
        </Text>
    ...
    >
      <View className="flex-row items-center gap-1.5">
        ...
        <Text
          className={
            compact
              ? "text-sm font-waypoint-medium text-foreground-muted"
              : "text-xs font-waypoint-medium text-foreground-muted"
          }
        >
          {label}
        </Text>
...
export const PendingTaskListRow = memo(function PendingTaskListRow(props: {
  ...
  const statusPill = isDraft ? (
    <View className="rounded-full bg-adaptive-amber-500-a12-a16 px-1.5 py-0.5">
      <Text className="text-3xs font-waypoint-bold text-adaptive-amber-700-300">Draft</Text>
    </View>
  ) : (
    <View className="rounded-full bg-subtle px-1.5 py-0.5">
      <Text className="text-3xs font-waypoint-bold text-foreground-muted">Pending</Text>
    </View>
  );
  ...
  const rowContent = compact ? (
      ...
      <View className="pr-[18px] pt-[10px]" style={{ paddingLeft: THREAD_LIST_COMPACT_INSET }}>
        <View className={cn("gap-[3px] pb-[10px]", !props.isLast && "border-b border-separator")}>
          <View className="flex-row items-center justify-between gap-2">
            <Text className="flex-1 text-lg font-waypoint-bold text-foreground" numberOfLines={1}>
              {pendingTask.title}
            </Text>
    ...
    >
      <View className="gap-[3px]">
        <View className="flex-row items-center justify-between gap-2">
          <Text className="flex-1 text-base font-waypoint-medium text-foreground" numberOfLines={1}>
            {pendingTask.title}
          </Text>
          <View className="flex-row items-center gap-2">
            {statusPill}
...
export const ThreadListRow = memo(function ThreadListRow(props: {
  ...
  const statusPill = effectiveStatus ? (
    <View className={`${effectiveStatus.pillClassName} rounded-full px-1.5 py-0.5`}>
      <Text className={`text-3xs font-waypoint-bold ${effectiveStatus.textClassName}`}>
      ...
      <View className="mt-px flex-row items-center gap-1.5">
        ...
        {pr !== null ? (
          ...
          >
            ...
            <Text
              className={`${compact ? "text-sm" : "text-xs"} font-waypoint-medium ${
                visuallySelected
                  ? materialYouStyleLayoutActive
                    ? "text-thread-selected-foreground"
                    : "text-user-bubble-foreground"
        ...
        <View className="pr-[18px] pt-[10px]" style={{ paddingLeft: THREAD_LIST_COMPACT_INSET }}>
          <View className={cn("gap-[3px] pb-[10px]", !props.isLast && "border-b border-separator")}>
            <View className="flex-row items-center justify-between gap-2">
              <Text
                className={cn(
                  "flex-1 text-lg font-waypoint-bold",
                  visuallySelected
                    ? materialYouStyleLayoutActive
                      ? "text-thread-selected-foreground"
                      : "text-user-bubble-foreground"
      ...
      >
        <View className="gap-[3px]">
          <View className="flex-row items-center justify-between gap-2">
            <Text
              className={cn(
                "flex-1 text-base font-waypoint-medium",
                visuallySelected
                  ? materialYouStyleLayoutActive
                    ? "text-thread-selected-foreground"
                    : "text-user-bubble-foreground"
