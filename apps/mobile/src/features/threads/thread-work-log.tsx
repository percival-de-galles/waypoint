...
import type { EnvironmentId, ToolActivityIcon } from "@waypoint/contracts";
import { toolActivityFaviconUrl } from "@waypoint/shared/favicon";

import { AppText as Text } from "../../components/AppText";
import { WaypointWordmark } from "../../components/WaypointWordmark";
...
type WorkContentIcon = AppSymbolName | "browser" | "device" | "waypoint" | "pull-request";
...
function WorkLogIcon(props: {
  ...
  if (props.icon === "waypoint") {
    return (
      <WaypointWordmark height={10} {...(colorClassName ? { colorClassName } : { color: props.color })} />
    );
  }
  return (
    <SymbolView
      ...
      >
        <View className="min-h-8 flex-row items-center gap-1.5">
            ...
            <>
              ...
              <Text
                className={cn(
                  ...
                  iconIsDestructive && "font-waypoint-medium text-adaptive-rose-600-400",
                )}
          ...
          <View className="shrink-0 flex-row items-center gap-px">
            {props.copied ? (
              <Text className="pr-1 font-waypoint-medium text-3xs text-adaptive-emerald-600-400">
                Copied
              </Text>
            ) : null}
...
export function ThreadWorkGroupToggle(props: {
  ...
  readonly summaryToolIcon?: "browser" | "device" | "waypoint" | "pull-request";
  readonly themeAppearance: "light" | "dark";
  readonly toolSurface?: import("@waypoint/contracts").ToolActivitySurface;
  readonly toolIcon?: ToolActivityIcon;
      ...
      >
        <View className="flex-row items-center gap-2">
          ...
          <View className="min-w-0 flex-1 gap-0.5">
            <Text
              key={props.rowSizing.textSizeKey}
              className="font-waypoint-medium text-sm text-foreground"
              numberOfLines={1}
            >
              {summary.title}
            </Text>
