...
describe("isLegalDocumentUrl", () => {
  it.each([
    "https://waypoint.invalid/legal",
    "https://waypoint.invalid/legal/",
    "https://waypoint.invalid/privacy-policy?source=app",
    "https://waypoint.invalid/terms-of-service#updates",
    "https://waypoint.invalid/security-policy",
  ])("allows a configured legal document: %s", (url) => {
    expect(isLegalDocumentUrl(url)).toBe(true);
  });

  it.each([
    "https://waypoint.invalid/download",
