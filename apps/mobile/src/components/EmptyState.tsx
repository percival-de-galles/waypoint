...
export function EmptyState(props: {
  ...
  if (props.variant === "plain") {
    return (
      <View className="items-center px-8 py-8">
        <Text className="text-center text-xl font-waypoint-bold text-foreground">{props.title}</Text>
        ...
        {props.actionLabel && props.onAction ? (
          ...
          >
            <Text className="text-sm font-waypoint-bold text-primary-foreground">
              {props.actionLabel}
            </Text>
          </Pressable>
        ) : null}
  ...
  return (
    <View className="rounded-[22px] border border-border bg-card p-5">
      <Text className="font-waypoint-bold text-lg text-foreground">{props.title}</Text>
      ...
      {props.actionLabel && props.onAction ? (
        <Pressable
          ...
          onPress={props.onAction}
        >
          <Text className="text-sm font-waypoint-bold text-primary-foreground">{props.actionLabel}</Text>
        </Pressable>
      ) : null}
    </View>
  );
