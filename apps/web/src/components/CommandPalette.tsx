...
import { threadPullRequestLinkMode } from "@waypoint/client-runtime/thread-pull-request-compatibility";
...
} from "@waypoint/client-runtime/operations/projects";
import { connectionStatusText } from "@waypoint/client-runtime/connection";
import { threadSearchMatchKey } from "@waypoint/client-runtime/state/thread-search";
...
} from "@waypoint/client-runtime/state/filesystem";
import {
...
import { onOpenCommandPalette, openWorkspaceView } from "../commandPaletteBus";
...
function OpenCommandPaletteDialog(props: {
  ...
  if (projects.length > 0) {
    ...
    if (activeProjectTitle) {
      actionItems.push({
        kind: "action",
        value: "action:open-project-board",
        searchTerms: ["board", "kanban", "queue", "review", "project workspace"],
        title: "Open project board",
        description: activeProjectTitle,
        icon: <Columns3Icon className={ITEM_ICON_CLASS} />,
        shortcutCommand: "workspace.board.toggle",
        run: async () => {
          openWorkspaceView("board");
        },
      });
    }
