import type { EnvironmentId, ServerSelfUpdateCapability } from "@waypoint/contracts";
import type { ServerUpdateStage, ServerUpdateState } from "@waypoint/client-runtime/state/server";
import {
      ...
      toastManager.add({
        ...
        description:
          selfUpdate === "desktop-managed"
            ...
            : `Reconnected on waypoint@${result.value.targetVersion}.`,
      });
    } catch (error) {
      toastManager.add({
        type: "error",
      ...
      if (desktopTargets.length > 0) {
        const confirmed =
          (await requestConfirmDialog(
            `Update the Waypoint desktop apps on ${desktopTargets.map((target) => target.serverLabel).join(", ")}? They will close and relaunch on those machines.`,
          )) ?? true;
        if (!confirmed) return;
      }
      await Promise.all(
...
export function ServerUpdateAction({
  ...
  const handleUpdate = async () => {
    ...
    if (isDesktopAppUpdate) {
      ...
      const confirmed =
        (await requestConfirmDialog(
          `Update the Waypoint desktop app that runs the ${serverLabel}? It will close and relaunch on that machine.`,
        )) ?? true;
      if (!confirmed) {
        return;
      }
