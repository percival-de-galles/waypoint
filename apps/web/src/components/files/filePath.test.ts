...
describe("fileBreadcrumbs", () => {
  it("builds project, directory, and file crumbs", () => {
    expect(fileBreadcrumbs("waypoint", "apps/web/src/main.tsx")).toEqual([
      { label: "waypoint", path: "", kind: "project" },
  ...
  it("starts host paths outside the workspace at the filesystem root", () => {
    expect(fileBreadcrumbs("waypoint", "/tmp/waypoint-cleanup/report.md")).toEqual([
      { label: "tmp", path: "/tmp", kind: "directory" },
      { label: "waypoint-cleanup", path: "/tmp/waypoint-cleanup", kind: "directory" },
      { label: "report.md", path: "/tmp/waypoint-cleanup/report.md", kind: "file" },
    ]);
    expect(fileBreadcrumbs("waypoint", "C:\\Temp\\report.md")).toEqual([
      { label: "C:", path: "C:", kind: "directory" },
      { label: "Temp", path: "C:\\Temp", kind: "directory" },
      { label: "report.md", path: "C:\\Temp\\report.md", kind: "file" },
    ]);
    expect(fileBreadcrumbs("waypoint", "\\\\server\\share\\report.md").map((c) => c.path)).toEqual([
      "\\\\server",
      "\\\\server\\share",
