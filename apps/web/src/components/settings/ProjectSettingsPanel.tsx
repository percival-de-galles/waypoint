...
import { scopeProjectRef, scopeThreadRef } from "@waypoint/client-runtime/environment";
...
import { resolveProjectAutoPull } from "@waypoint/shared/serverSettings";
import {
  projectScriptsInheritDefaults,
  resolveProjectScripts,
} from "@waypoint/shared/projectScripts";
import { DEFAULT_RESOLVED_KEYBINDINGS } from "@waypoint/shared/keybindings";
...
import { useWaypointProjectFileState } from "../../hooks/useWaypointProjectFileScripts";
...
function ProjectDetail({
  ...
  const waypointFile = useWaypointProjectFileState(
  ...
  // repo's waypoint.json value when present, otherwise the global setting.
  const inheritedEnvMode = waypointFile.file?.defaultThreadEnvMode ?? scriptSettings.defaultThreadEnvMode;
  const inheritedEnvModeSource = waypointFile.file?.defaultThreadEnvMode != null ? "waypoint.json" : "global";
  const importableScripts = useMemo(
    () =>
      waypointFile.scripts.filter(
        (fileScript) =>
          !scripts.some(
            (script) =>
  ...
  const importFileScript = useCallback(
    async (fileScript: WaypointProjectFileScript) => {
      ...
      <SettingsPageContainer className="gap-6">
        <SettingsSection title="Project" hideTitle>
          ...
          <SettingsRow
            ...
            description={
              storedEnvMode === null
                ? "Inherited from waypoint.json or machine defaults."
              ...
              >
                ...
                <SelectPopup align="end" alignItemWithTrigger={false}>
                  <SelectItem value="inherit">
                    {group.memberProjects.length > 1
                      ? "Default (each checkout's waypoint.json or global setting)"
        ...
        <SettingsSection title="Checkout">
              ...
              {importableScripts.length > 0 ? (
                <Menu>
                  ...
                  <MenuPopup align="end" className="w-72">
                    <MenuGroup>
                      <MenuGroupLabel>Import from waypoint.json</MenuGroupLabel>
          ...
          />
          {waypointFile.status === "invalid" ? (
            <SettingsRow
              title="waypoint.json is invalid"
              description="A waypoint.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
