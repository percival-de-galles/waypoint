...
function categoryLabel(category: ResourceTelemetryProcessCategory): string {
  switch (category) {
    ...
    case "resource-monitor":
      return "Monitor";
    case "unknown-waypoint":
      return "Waypoint process";
  }
}
...
export function ResourceTelemetryDiagnostics() {
  ...
  const allWaypoint = snapshot?.groups.allWaypoint;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allWaypoint ? `${allWaypoint.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={allWaypoint ? `${formatCpuTime(allWaypoint.cpuTimeMs)} observed CPU time` : undefined}
            />
            <IconStat
              icon={<MemoryStickIcon className="size-3.5" />}
              label="Resident memory"
              value={allWaypoint ? formatBytes(allWaypoint.currentRssBytes) : "..."}
              detail={
                allWaypoint ? `${formatBytes(allWaypoint.peakRssBytes)} combined process peaks` : undefined
              }
            />
            <IconStat
              icon={<ActivityIcon className="size-3.5" />}
              label="Process count"
              value={allWaypoint ? String(allWaypoint.processCount) : "..."}
              detail={
                allWaypoint ? `${allWaypoint.processStarts} starts · ${allWaypoint.processExits} exits` : undefined
              }
            />
            <IconStat
              icon={<HardDriveIcon className="size-3.5" />}
              label="Read throughput"
              value={allWaypoint ? formatRate(allWaypoint.ioReadBytesPerSecond) : "..."}
              detail={allWaypoint ? `${formatBytes(allWaypoint.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allWaypoint ? formatRate(allWaypoint.ioWriteBytesPerSecond) : "..."}
              detail={allWaypoint ? `${formatBytes(allWaypoint.ioWriteBytes)} observed` : undefined}
              tone={
                allWaypoint && allWaypoint.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allWaypoint && allWaypoint.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known Waypoint operations so process spikes can be correlated with specific
