...
import {
  ...
  hasQuestionAnswer,
} from "@waypoint/client-runtime/work-log/user-input";
import {
...
import { parseScopedThreadKey } from "@waypoint/client-runtime/environment";
import type { CodexArtifactTemplate } from "@waypoint/client-runtime/codex-artifact-templates";
import {
  resolveWorkEntryToolPresentation,
  resolveViewedImageAsset,
  workEntryViewedImagePath,
} from "@waypoint/client-runtime/work-log/presentation";
import { resolveWorkGroupScrollAnchor } from "@waypoint/client-runtime/work-log/scroll-anchor";
import type { AgentPanelModel } from "@waypoint/client-runtime/state/subagentRuntime";
import {
...
import { resolveChatListAnchoredEndSpace } from "@waypoint/shared/chatList";
import { toolActivityFaviconUrl } from "@waypoint/shared/favicon";
import { formatDuration } from "@waypoint/shared/orchestrationTiming";
import { getProjectFaviconCacheKey } from "@waypoint/shared/projectFavicon";
...
import { WaypointWordmark } from "../WaypointWordmark";
import {
  BotIcon,
  BrainIcon,
  CheckIcon,
...
import { type TimestampFormat } from "@waypoint/contracts/settings";
...
function WorkGroupToggleTimelineRow({
  ...
  return (
    <button
      type="button"
      className={cn(
        "group/tool-group flex min-h-6 w-full cursor-pointer items-center gap-1.5 rounded-md border border-transparent px-1 py-0.5 text-left text-sm leading-relaxed transition-[background-color,border-color] duration-150 hover:bg-accent/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/70",
        row.expanded && "border-border/55 bg-muted/30",
        row.hasFailure && "border-destructive/20 bg-destructive/4",
      )}
...
function WorkEntryIcon({ name, className }: { name: WorkEntryIconName; className: string }) {
  switch (name) {
    ...
    case "waypoint":
      return <WaypointWordmark className={className} aria-hidden />;
    case "check":
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-md border border-transparent px-1 transition-[background-color,border-color] duration-150",
        isExpandedToolGroupEntry ? "py-0" : "py-0.5",
        expanded && workLogEntryIsToolLike(workEntry) && "border-border/55 bg-muted/30",
        canExpand &&
    ...
    >
      ...
      {expanded && canExpand && expandedBody && !workEntry.questionAnswer ? (
        <div
          className="mt-1 ms-7 cursor-default rounded-md border border-border/50 bg-background/55 px-3 py-2"
          onClick={stopRowToggle}
          onPointerDown={stopRowToggle}
        >
...
function QuestionAnswerHistory({
  ...
  answer: import("@waypoint/contracts").UserInputAttachmentAnswerPayload;
}) {
  const { activeThreadEnvironmentId } = use(TimelineRowCtx);
