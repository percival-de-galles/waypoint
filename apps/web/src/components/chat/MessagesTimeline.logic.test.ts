...
describe("streaming row projection", () => {
  ...
  it("owns checkpoint lookups across streaming and equal source snapshots", () => {
    ...
    const summary: TurnDiffSummary = {
      ...
      checkpointRef: CheckpointRef.make("refs/waypoint/checkpoints/history-turn"),
      status: "ready",
      files: [],
  ...
  it("reuses long-thread rows through detail events, selectors, and attachment previews", () => {
    ...
    const history = Array.from({ length: 250 }, (_, index) => {
      ...
      const checkpoint: TurnDiffSummary = {
        ...
        checkpointRef: CheckpointRef.make(`refs/waypoint/checkpoints/older-${index}`),
        status: "ready",
        files: [],
        get assistantMessageId() {
          checkpointLookupReads += 1;
...
describe("work entry labels", () => {
  ...
  it.each([
    ...
    const browserEntry = {
      ...entry,
      toolTitle: "Waypoint-code.preview_click",
      detail: '{"ok":true}',
      toolLifecycleStatus,
    };
  ...
  it("uses the active summary state for legacy tools without a lifecycle status", () => {
    const browserEntry = { ...entry, toolTitle: "Waypoint-code.preview_click" };
    ...
    (isWorking) => {
      ...
      const timelineEntries = Array.from({ length: 1_000 }, (_, index) => ({
        ...
        entry: {
          ...
          createdAt,
          turnId,
          label: "waypoint.preview_snapshot",
          tone: "tool" as const,
          toolLifecycleStatus:
