...
describe("MessagesTimeline", () => {
  ...
  it("anchors the first user message using its measured height", () => {
    ...
    expect(markup).toContain("waypoint — Tests");
  ...
  it("does not render window details before the preview URL resolves", () => {
    ...
    expect(markup).not.toContain("waypoint — Tests");
  ...
  it("renders unsafe user HTML as inert source text", async () => {
    ...
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          buildUserTimelineEntry(
            '<script>globalThis.__waypointXss = 1</script><img src="x" onerror="globalThis.__waypointXss = 2">',
          ),
        ]}
      />,
    );

    expect(markup).toContain("&lt;script&gt;globalThis.__waypointXss = 1&lt;/script&gt;");
    expect(markup).toContain(
      "&lt;img src=&quot;x&quot; onerror=&quot;globalThis.__waypointXss = 2&quot;&gt;",
    );
  ...
  it("sanitizes executable HTML while preserving supported assistant markup", async () => {
      ...
      <MessagesTimeline
        ...
        timelineEntries={[
          buildAssistantTimelineEntry(
            [
              '<details open onclick="globalThis.__waypointXss = 1">',
              "<summary>Safe details</summary>",
              "<script>globalThis.__waypointXss = 2</script>",
              '<img src="x" onerror="globalThis.__waypointXss = 3">',
              '<a href="javascript:globalThis.__waypointXss = 4">Unsafe link</a>',
              "</details>",
            ].join(""),
          ),
        ]}
    ...
    expect(markup).not.toContain("globalThis.__waypointXss");
  });
        ...
        timelineEntries={[
          {
            ...
            entry: {
              ...
              label: "Updated files",
              tone: "tool",
              changedFiles: ["C:/Users/mike/dev-stuff/waypoint/apps/web/src/session-logic.ts"],
            },
          },
        ]}
        workspaceRoot="C:/Users/mike/dev-stuff/waypoint"
      />,
    );

    expect(markup).toContain("Changed 1 file");
    expect(markup).not.toContain("C:/Users/mike/dev-stuff/waypoint/apps/web/src/session-logic.ts");
  });
