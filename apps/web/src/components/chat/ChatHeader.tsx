...
import { ChevronDownIcon, Columns3Icon, MessageSquareIcon } from "lucide-react";
import {
  memo,
  useCallback,
  useEffect,
...
import { useWaypointProjectFileScripts } from "~/hooks/useWaypointProjectFileScripts";
...
interface ChatHeaderProps {
  ...
  workspaceView: "chat" | "board";
  onWorkspaceViewChange: (view: "chat" | "board") => void;
  gitCwd: string | null;
...
export const ChatHeader = memo(function ChatHeader({
  ...
  const fileScripts = useWaypointProjectFileScripts(
    activeThreadEnvironmentId,
  ...
  return (
    ...
    >
      ...
      >
        {activeProject ? (
          <div className="flex items-center rounded-md border border-border/70 bg-muted/35 p-0.5">
            <Tooltip>
              <TooltipTrigger
                render={
                  <button
                    ...
                    className={cn(
                      "inline-flex size-6 cursor-pointer items-center justify-center rounded-[calc(var(--radius-md)-2px)] text-muted-foreground transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring",
                      workspaceView === "chat" && "bg-background text-foreground shadow-xs",
                    )}
                  />
                }
              >
              ...
              <TooltipPopup side="top">Chat</TooltipPopup>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger
                render={
                  <button
                    ...
                    className={cn(
                      "inline-flex size-6 cursor-pointer items-center justify-center rounded-[calc(var(--radius-md)-2px)] text-muted-foreground transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring",
                      workspaceView === "board" && "bg-background text-foreground shadow-xs",
                    )}
                  />
                }
              >
              ...
              <TooltipPopup side="top">Project board</TooltipPopup>
            </Tooltip>
          </div>
        ) : null}
        {activeProjectScripts && (
