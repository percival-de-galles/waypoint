...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@waypoint/contracts";
      ...
      projects: [
        {
          ...
          title: "Waypoint",
          workspaceRoot: "/Users/theo/Projects/waypoint",
        },
        {
          environmentId: remoteEnvironmentId,
          title: "waypoint",
          workspaceRoot: "/srv/waypoint",
        },
      ],
    ...
    const metadata = buildCommandPaletteProjectMetadata({
      projects: [
        ...
        },
        {
          ...
          workspaceRoot: "/srv/waypoint-feature",
        },
      ],
      locationByEnvironmentId: locations,
    });
    ...
    const metadata = buildCommandPaletteProjectMetadata({
      projects: [
        ...
        },
        {
          ...
          title: "Waypoint mirror",
          workspaceRoot: "/srv/mirror/waypoint",
        },
      ],
...
describe("buildProjectActionItems", () => {
  it("shows the grouped display name but keeps the real title for icons", () => {
    ...
    const [item] = buildProjectActionItems({
      projects: [{ ...project, displayName: "waypointdotgg/fleet" }],
      valuePrefix: "project",
      icon: (candidate) => {
    ...
    expect(item?.title).toBe("waypointdotgg/fleet");
    expect(item?.searchTerms).toEqual(
      expect.arrayContaining(["waypointdotgg/fleet", "fleet", "/Users/theo/Code/p/fleet"]),
    );
...
describe("buildThreadActionItems", () => {
  ...
  it("keeps message excerpts searchable without replacing thread metadata", () => {
    const [item] = buildThreadActionItems({
      ...
      projectTitleById: new Map([[PROJECT_ID, "Waypoint"]]),
      sortOrder: "updated_at",
      icon: null,
      getContentMatch: () => ({
        source: "assistant",
    ...
    expect(item?.description).toBe("Waypoint · #feat/search");
  });
  ...
  it("prefers renderDescription when provided", () => {
    ...
    expect(item?.description).toBe("Waypoint:feat/search:wt");
  });
...
it.each([
  "#10839",
  "10839",
  "pingdotgg/waypoint#10839",
  "https://github.com/pingdotgg/waypoint/pull/10839",
        ...
        pullRequests: [
          {
            ...
            number: 10839,
            url: "https://github.com/pingdotgg/waypoint/pull/10839",
            source: "manual",
