import { EnvironmentId, type VcsRef } from "@waypoint/contracts";
  ...
  it("picks the most recently updated worktree thread", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "waypoint/older",
            worktreePath: "/repo/.waypoint/worktrees/older",
            updatedAt: "2026-07-20T00:00:00.000Z",
          },
          {
            branch: "waypoint/newer",
            worktreePath: "/repo/.waypoint/worktrees/newer",
    ...
    ).toEqual({ branch: "waypoint/newer", worktreePath: "/repo/.waypoint/worktrees/newer" });
  });
  ...
  it("skips the worktree the composer already points at", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "waypoint/current",
            worktreePath: "/repo/.waypoint/worktrees/current",
            updatedAt: "2026-07-22T00:00:00.000Z",
          },
        ],
        currentWorktreePath: "/repo/.waypoint/worktrees/current",
      }),
    ).toBeNull();
  });
  ...
  it("ignores archived threads and threads with unparseable timestamps", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "waypoint/archived",
            worktreePath: "/repo/.waypoint/worktrees/archived",
            updatedAt: "2026-07-23T00:00:00.000Z",
            archivedAt: "2026-07-23T01:00:00.000Z",
          },
          {
            branch: "waypoint/garbage-timestamp",
            worktreePath: "/repo/.waypoint/worktrees/garbage",
            updatedAt: "not-a-date",
          },
          {
            branch: "waypoint/live",
            worktreePath: "/repo/.waypoint/worktrees/live",
    ...
    ).toEqual({ branch: "waypoint/live", worktreePath: "/repo/.waypoint/worktrees/live" });
  });
});

describe("resolvePreviousWorktreeLabel", () => {
  it("includes the branch when known", () => {
    expect(resolvePreviousWorktreeLabel({ branch: "waypoint/fix-thing", worktreePath: "/wt" })).toBe(
      "Previous worktree (waypoint/fix-thing)",
    );
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.waypoint/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a ref already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.waypoint/worktrees/feature-a",
        currentWorktreePath: null,
...
describe("resolveLocalCheckoutBranchMismatch", () => {
  ...
  it("ignores dedicated worktrees because their checkout is already thread-scoped", () => {
    expect(
      resolveLocalCheckoutBranchMismatch({
        effectiveEnvMode: "worktree",
        activeWorktreePath: "/repo/.waypoint/worktrees/feature-thread",
        activeThreadBranch: "feature/thread",
        currentGitBranch: "feature/current",
      }),
    ).toBeNull();
...
describe("resolveEffectiveEnvMode", () => {
  it("treats draft threads already attached to a worktree as current-checkout mode", () => {
    expect(
      resolveEffectiveEnvMode({
        activeWorktreePath: "/repo/.waypoint/worktrees/feature-a",
...
describe("resolveCurrentWorkspaceLabel", () => {
  ...
  it("describes the active checkout as a worktree when one is attached", () => {
    expect(resolveCurrentWorkspaceLabel("/repo/.waypoint/worktrees/feature-a")).toBe("Current worktree");
  });
});
...
describe("resolveLockedWorkspaceLabel", () => {
  ...
  it("uses a shorter label for an attached worktree", () => {
    expect(resolveLockedWorkspaceLabel("/repo/.waypoint/worktrees/feature-a")).toBe("Worktree");
  });
});
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected ref", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        refName: {
          isDefault: false,
          worktreePath: "/repo/.waypoint/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.waypoint/worktrees/feature-b",
      nextWorktreePath: "/repo/.waypoint/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default refs", () => {
    expect(
      ...
      checkoutCwd: "/repo/.waypoint/worktrees/feature-a",
      nextWorktreePath: "/repo/.waypoint/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
