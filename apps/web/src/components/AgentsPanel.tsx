...
import type { EnvironmentId, ThreadId } from "@waypoint/contracts";
import { Activity, Bot, Braces, Check, ChevronDown, ChevronRight, X } from "lucide-react";
...
function AgentRow({ agent }: { agent: RuntimeSubagent }) {
  ...
  return (
    <div className="waypoint-activity-enter rounded-md motion-reduce:animate-none">
      <div className="grid h-[3.875rem] grid-cols-[0.375rem_minmax(0,1fr)_auto] grid-rows-[1.25rem_1.125rem_1rem] items-center gap-x-2 px-1.5 py-1">
        ...
        <span className="col-start-3 row-start-1 flex min-w-14 items-center justify-end gap-1 text-right font-mono text-[.7rem] text-muted-foreground/80">
          ...
          {agent.recentActivity.length > 0 ? (
            <Button
              size="icon-tiny"
              variant="ghost-muted"
              aria-label={`${activityOpen ? "Hide" : "Show"} activity for ${agent.title}`}
      ...
      {activityOpen ? (
        <section
          aria-label={`Recent activity for ${agent.title}`}
          className="waypoint-activity-enter mb-1 ml-3.5 rounded-md border border-border/55 bg-muted/25 px-2 py-1.5 motion-reduce:animate-none"
        >
          <div className="mb-1 flex items-center gap-1 text-[.65rem] font-medium uppercase tracking-wider text-muted-foreground">
            <Activity aria-hidden className="size-3" />
            Activity
          </div>
          <ol className="space-y-1">
            {agent.recentActivity.map((entry) => (
              <li key={`${entry.at}:${entry.summary}`} className="flex gap-2 text-xs leading-4">
                <span className="mt-1.5 size-1 shrink-0 rounded-full bg-muted-foreground/45" />
                <span className="min-w-0 text-muted-foreground">{entry.summary}</span>
              </li>
            ))}
          </ol>
        </section>
