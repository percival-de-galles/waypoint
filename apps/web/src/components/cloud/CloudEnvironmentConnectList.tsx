...
/**
 * The user's Waypoint Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Waypoint Connect.`,
      });
      return true;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Waypoint Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[waypoint-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
