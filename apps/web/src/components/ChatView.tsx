...
import type { UsageLimitSourceSnapshots } from "@waypoint/contracts";
import {
  collectProviderUsageLimits,
  hasProviderUsageLimits,
  isUsageLimitsCommand,
} from "@waypoint/shared/usageLimits";
import { feedbackBannerItem } from "./chat/ComposerFeedback";
import { usageLimitsBannerItem } from "./chat/ComposerUsageLimits";
import { derivePendingRequests } from "@waypoint/client-runtime/pending-requests";
import {
...
import { type EnvironmentConnectionPresentation } from "@waypoint/client-runtime/connection";
import { wasBootstrapThreadDeleted } from "@waypoint/client-runtime/errors";
import { type CodexArtifactTemplate } from "@waypoint/client-runtime/codex-artifact-templates";
import { effectiveSnoozed, threadWokeAt } from "@waypoint/client-runtime/state/thread-settled";
import {
  parseCodexFeedbackCommand,
  submitCodexFeedback,
  type CodexFeedbackSubmission,
} from "@waypoint/client-runtime/state/threads";
import {
  parseScopedThreadKey,
  scopedThreadKey,
  scopeProjectRef,
  scopeThreadRef,
} from "@waypoint/client-runtime/environment";
import {
...
} from "@waypoint/shared/projectScripts";
import { truncate } from "@waypoint/shared/String";
import { resolveThreadReferenceCopyTarget } from "@waypoint/shared/threadReference";
import {
  getTerminalLabel,
  nextTerminalId,
  resolveTerminalSessionLabel,
} from "@waypoint/shared/terminalLabels";
...
import { assistantCitationsToPlainText } from "@waypoint/shared/assistantCitations";
...
import { onOpenWorkspaceView } from "../commandPaletteBus";
import { subscribeSnapShotComposerFocus } from "../lib/desktopSnapShot";
import { buildTemporaryWorktreeBranchName } from "@waypoint/shared/git";
...
} from "@waypoint/client-runtime/state/subagentRuntime";
...
import { resolveProviderSkillsForCwd } from "@waypoint/client-runtime/providerSkills";
...
import { clampFileAttachmentUploadBytes } from "@waypoint/client-runtime/state/attachments";
...
export default function ChatView(props: ChatViewProps) {
  ...
  useEffect(() => onOpenWorkspaceView(setWorkspaceView), []);
  ...
  // Waypoint owns /usage-limits only where Limits has data for the selected provider;
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `waypoint:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  // the sent message can stay near the top. Waypoint only owns streaming adjustments
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      if (command === "workspace.board.toggle") {
        event.preventDefault();
        event.stopPropagation();
        if (activeProject) setWorkspaceView((view) => (view === "board" ? "chat" : "board"));
        return;
      }
  ...
  const onRespondToUserInput = useCallback(
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      ...
      const attachmentsByQuestionId = new Map<
        string,
        import("@waypoint/contracts").UserInputAttachments[string]
      >();
      for (const question of activePendingUserInput.questions) {
        ...
        attachmentsByQuestionId.set(
          question.id,
          uploaded as import("@waypoint/contracts").UserInputAttachments[string],
        );
      }
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Waypoint server to browse pull requests."
      />
