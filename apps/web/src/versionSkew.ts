import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@waypoint/contracts";
import type { ServerUpdateState } from "@waypoint/client-runtime/state/server";
...
const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "waypoint:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older Waypoint than
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Waypoint version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx waypoint@${targetVersion}`;
}
