...
} from "@waypoint/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@waypoint/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@waypoint/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@waypoint/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@waypoint/client-runtime/rpc";
import {
    ...
    const cloudSession = CloudSession.of({
      ...
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Waypoint Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Waypoint Connect session is unavailable.",
          });
        }
        return token;
      }),
