...
describe("terminalLinkAtPositionWithRange", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://waypoint.invalid", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtPositionWithRange([row], 0, 2)?.text).toBe("https://waypoint.invalid");
    expect(terminalLinkAtPositionWithRange([row], 0, cells.length - 1)?.text).toBe(
      "https://waypoint.invalid",
    );
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://waypoint.invalid", false, true)];
    expect(terminalLinkAtPositionWithRange(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://waypoint.invalid", false), row("", false)];
    expect(terminalLinkAtPositionWithRange(complete, 0, 8)?.text).toBe("https://waypoint.invalid");
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell("🙂"), wide: 1 },
        { ...cell(""), wide: 2 },
        ...Array.from("https://waypoint.code", (character) => cell(character)),
      ],
      text: "🙂 https://waypoint.code",
      isWrapContinuation: false,
      wrapsToNext: true,
    };
    ...
    const unwrittenTail: GhosttyRow = {
      ...
      text: "https://waypoint.invalid",
      isWrapContinuation: false,
      wrapsToNext: false,
    };
    expect(terminalLinkAtPositionWithRange([unwrittenTail], 0, 8)?.text).toBe("https://waypoint.invalid");
  });
});
