...
/**
 * Drives the primary environment's Waypoint Connect link. Waypoint Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update Waypoint Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[waypoint-connect] Could not update Waypoint Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Waypoint Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Waypoint Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Waypoint Connect before enabling this."));
        return false;
      }
