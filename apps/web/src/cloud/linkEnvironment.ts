...
import { EnvironmentRegistry } from "@waypoint/client-runtime/connection";
import { request, runStream } from "@waypoint/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@waypoint/client-runtime/rpc";
import { ManagedRelay, relayProtectedErrorMessage } from "@waypoint/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Waypoint cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Waypoint cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of Waypoint Connect while clients reach the environment out of band.
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "WAYPOINT_RELAY_URL is not configured.",
      });
    }
