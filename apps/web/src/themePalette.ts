...
import {
  ...
  type ThemeVariants,
} from "@waypoint/shared/themePalettes";

export { EMBER_THEME, GROVE_THEME, IRIS_THEME, OCEAN_THEME, WAYPOINT_CHAT_THEME, THEME_COLOR_ROLES };
export type { ThemeAppearance, ThemeColorRole, ThemeColors, ThemeDefinition, ThemeVariants };

export const WAYPOINT_CHAT_THEME_ID = "waypoint-chat" as const;
...
export const CUSTOM_THEMES_STORAGE_KEY = "waypoint:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "waypoint:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "waypoint:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "waypoint:theme-halves:v1";

const LEGACY_WAYPOINT_CHAT_DARK_THEME_ID = "waypoint-chat-dark";
...
// Earlier builds shipped every maintainer theme under a waypoint- prefix; only the
// genuinely Waypoint-branded palette keeps it. Stored preferences and mixes with the
// old ids stay readable through this alias table.
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_WAYPOINT_CHAT_DARK_THEME_ID]: WAYPOINT_CHAT_THEME_ID,
  "waypoint-grove": GROVE_THEME_ID,
  "waypoint-ocean": OCEAN_THEME_ID,
  "waypoint-ember": EMBER_THEME_ID,
  "waypoint-iris": IRIS_THEME_ID,
};
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_WAYPOINT_CHAT_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
// Older builds stored the dark Waypoint Chat palette as a separate theme. Keep
...
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_WAYPOINT_CHAT_DARK_THEME_ID ? "dark" : null;
}

/**
 * The palette Waypoint wears with no theme installed, captured from the app's
 ...
 */
const WAYPOINT_CODE_LIGHT_THEME_COLORS: ThemeColors = {
  canvas: "#fcfcfc",
  chrome: "#fcfcfc",
  toolbar: "#fcfcfc",
...
const WAYPOINT_CODE_DARK_THEME_COLORS: ThemeColors = {
  canvas: "#0a0a0a",
  chrome: "#0a0a0a",
  toolbar: "#0a0a0a",
...
/**
 * The standard Waypoint look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Waypoint Chat palette used to fill roles omitted by theme
 * files.
 */
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(WAYPOINT_CODE_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(WAYPOINT_CODE_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors Waypoint shows without a theme, read from the app's own
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return appearance === "dark" ? WAYPOINT_CHAT_THEME.variants!.dark! : WAYPOINT_CHAT_THEME.colors;
}
