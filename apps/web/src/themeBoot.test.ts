...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Waypoint Chat follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "waypoint-chat", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Waypoint Chat",
      storage: {
        [THEME_STORAGE_KEY]: "waypoint-chat",
    ...
    {
      name: "a legacy waypoint-grove preference resolves through the alias",
      storage: { [THEME_STORAGE_KEY]: "waypoint-grove", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "legacy waypoint-chat-dark resolves to dark Waypoint Chat",
      storage: { [THEME_STORAGE_KEY]: "waypoint-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("waypoint-chat");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "waypoint:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("waypoint-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(WAYPOINT_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "waypoint-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "waypoint:theme-halves:v1": JSON.stringify({ dark: "waypoint-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "waypoint-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "waypoint:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("waypoint-chat");
    expect(boot.isDark).toBe(true);
  });
