import { EnvironmentId, ProjectId, type AgentSessionProjectCandidate } from "@waypoint/contracts";
...
describe("groupOnboardingProjects", () => {
  it("groups clones by origin, keeps local repos separate, and folds non-git folders away", () => {
    const main = candidate("/code/waypoint", {
      git: github("pingdotgg/waypoint"),
      threadCount: 79,
      lastActiveAt: "2026-08-21T12:00:00.000Z",
    });
    const clone = candidate("/code/clones/waypoint-2", {
      git: github("pingdotgg/waypoint"),
      threadCount: 13,
      lastActiveAt: "2026-08-10T12:00:00.000Z",
    });
    const older = candidate("/code/fleet", {
      git: github("waypointdotgg/fleet"),
      threadCount: 295,
    ...
    expect(
      ...
      {
        label: "waypointdotgg/fleet",
        paths: ["/code/fleet"],
        threadCount: 295,
        lastActiveAt: "2026-08-22T00:00:00.000Z",
      },
      {
        label: "pingdotgg/waypoint",
        paths: ["/code/waypoint", "/code/clones/waypoint-2"],
        threadCount: 92,
        lastActiveAt: "2026-08-21T12:00:00.000Z",
      },
      {
