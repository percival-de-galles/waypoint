...
import { BUILT_IN_THEMES } from "@waypoint/shared/themePalettes";
...
describe("theme files", () => {
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...WAYPOINT_CHAT_THEME,
      colors: { ...WAYPOINT_CHAT_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: WAYPOINT_CHAT_THEME.id,
      name: WAYPOINT_CHAT_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(WAYPOINT_CHAT_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(WAYPOINT_CHAT_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(WAYPOINT_CHAT_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(WAYPOINT_CHAT_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(WAYPOINT_CHAT_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(WAYPOINT_CHAT_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Waypoint Chat palette faithful and readable", () => {
    expectThemeColors(WAYPOINT_CHAT_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(WAYPOINT_CHAT_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(WAYPOINT_CHAT_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [WAYPOINT_CHAT_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy waypoint-chat-dark preference to dark Waypoint Chat", () => {
    expect(getThemeDefinition("waypoint-chat-dark")).toBe(WAYPOINT_CHAT_THEME);
    expect(getThemePreferenceMode("waypoint-chat-dark")).toBe("dark");
    expect(resolveThemeAppearance("waypoint-chat-dark", true, false)).toBe("dark");
    expect(resolveDesktopTheme("waypoint-chat-dark", false)).toBe("dark");
    expect(isKnownThemePreference("waypoint-chat-dark")).toBe(true);
  });

  it("resolves legacy waypoint-prefixed ids onto the renamed themes", () => {
    for (const [legacy, theme] of [
      ["waypoint-grove", GROVE_THEME],
      ["waypoint-ocean", OCEAN_THEME],
      ["waypoint-ember", EMBER_THEME],
      ["waypoint-iris", IRIS_THEME],
    ] as const) {
    ...
    expect(canonicalThemePreference("waypoint-chat-dark")).toBe("waypoint-chat-dark");
    // A stored mix that predates the rename resolves to the new ids.
    expect(parseThemeHalves(JSON.stringify({ light: "waypoint-ocean", dark: "waypoint-grove" }))).toEqual({
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", WAYPOINT_CHAT_THEME.id, GROVE_THEME.id]) {
...
describe("singleAppearanceOf", () => {
  it("reports the only half a theme can claim, and null for a pair", () => {
    const { variants: _pair, ...base } = WAYPOINT_CHAT_THEME;
    expect(singleAppearanceOf({ ...base, id: "x", appearance: "dark" })).toBe("dark");
    expect(singleAppearanceOf(WAYPOINT_CHAT_THEME)).toBe(null);
  });
});
