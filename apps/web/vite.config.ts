...
import { DEV_PROXIED_PATH_PREFIXES } from "@waypoint/shared/devProxy";
...
const isSingleOriginDev = process.env.WAYPOINT_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_WAYPOINT_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.WAYPOINT_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: WAYPOINT_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); WAYPOINT_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.WAYPOINT_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // WAYPOINT_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.WAYPOINT_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.WAYPOINT_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_WAYPOINT_RELAY_URL": JSON.stringify(configuredRelayUrl),
