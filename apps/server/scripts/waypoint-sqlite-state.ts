...
import { fromJsonStringPretty } from "@waypoint/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Waypoint once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.waypoint database. Use an isolated --base-dir.";
  }
}
...
const waypointSqliteStateCommand = Command.make(
  "waypoint-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Waypoint base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Waypoint SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(waypointSqliteStateCommand, { version: "0.0.0" }).pipe(
