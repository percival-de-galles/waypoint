...
describe("pair base URL selection", () => {
  ...
  it("pairs through the bound host when there is no dev server", () => {
    expect(resolveDirectPairingBaseUrl({ ...baseState, host: "100.64.0.7" })).toBe(
      "http://100.64.0.7:4773",
    );
    expect(resolveDirectPairingBaseUrl(baseState)).toBe("http://localhost:4773");
  });
});
  ...
  Effect.acquireUseRelease(
    Effect.callback<NodeHttp.Server>((resume) => {
      const server = NodeHttp.createServer((request, response) => {
        if (request.url === "/.well-known/waypoint/environment") {
...
describe("waypoint pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-pair-test-"));
        ...
        assert.equal(credentials[0]?.label, "waypoint pair");
      }),
    ).pipe(
  ...
  it.effect("pairs through the recorded dev web URL for dev servers", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-pair-dev-test-"));
        const port = Number(new URL(origin).port);
  ...
  it.effect("directs to waypoint serve or waypoint connect when no server is running", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-pair-none-test-"));
      ...
      assert.include(rendered, "No running Waypoint server found.");
      assert.include(rendered, "npx waypoint serve");
      assert.include(rendered, "npx waypoint connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("ignores runtime state whose recorded pid is no longer alive", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-pair-pid-test-"));
        const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
  ...
  it.effect("ignores stale runtime state pointing at a dead server", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-pair-stale-test-"));
      const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
