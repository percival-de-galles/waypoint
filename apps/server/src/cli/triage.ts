/**
 * `waypoint triage` - hand a misbehaving install to the user's own coding agent.
 *
...
import { isCommandAvailable, resolveSpawnCommand } from "@waypoint/shared/shell";
...
export const triageCommand = Command.make("triage", {
  ...
  Command.withDescription(
    "Investigate a Waypoint problem on this machine with claude or codex, and help file a good issue.",
  ),
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // --base-dir wins; WAYPOINT_HOME is its documented env equivalent (same
      // precedence as `waypoint pair`).
