...
import type { DesktopAppActivationRequest } from "@waypoint/contracts";
import { resolveDesktopAppControlAddress } from "@waypoint/shared/desktopAppControl";
import {
  HostProcessPlatform,
  HostProcessUserId,
...
describe("waypoint app", () => {
  it.effect("rejects SSH before it tries to reach a desktop app", () =>
    withTempDirectory("waypoint-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-waypoint-home");
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`waypoint app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
  ...
  it.effect("rejects unsupported platforms without creating state", () =>
    withTempDirectory("waypoint-app-platform-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-waypoint-home");
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`waypoint app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
  ...
  it.effect("does not create state when only a server or no desktop app is running", () =>
    withTempDirectory("waypoint-app-missing-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-waypoint-home");
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the Waypoint desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses WAYPOINT_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("waypoint-app-command-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "waypoint-home");
        ...
        yield* runCli(["app"], { WAYPOINT_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("waypoint-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".waypoint");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("finds the dev desktop when the default desktop socket is absent", () =>
    withTempDirectory("waypoint-app-dev-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".waypoint");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
        ...
        yield* runCli(["app"]);
        yield* runCli(["app"], { WAYPOINT_HOME: "   " });
  ...
  it.effect("never searches a dev state directory for an explicit Waypoint home", () =>
    withTempDirectory("waypoint-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".waypoint");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
        ...
        const envError = yield* runCli(["app"], { WAYPOINT_HOME: baseDir }).pipe(Effect.flip);
  ...
  for (const responseKind of ["failure", "invalid"] as const) {
    it.effect(`never falls back after the default desktop sends a ${responseKind} response`, () =>
      withTempDirectory("waypoint-app-response-test-", (root) =>
        Effect.gen(function* () {
          vi.mocked(NodeOS.homedir).mockReturnValue(root);
          const baseDir = NodePath.join(root, ".waypoint");
