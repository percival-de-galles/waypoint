/**
 * All text `waypoint triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Waypoint triage playbook

You are a support engineer for Waypoint (https://github.com/pingdotgg/waypoint), working
...
Fetch https://raw.githubusercontent.com/pingdotgg/waypoint/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/pingdotgg/waypoint <source-cache-dir>/<hash>
...
- How is Waypoint running on this machine: \`npx waypoint serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (waypoint.invalid), the
...
Search existing issues in pingdotgg/waypoint (use \`gh\`, or the public GitHub search
...
Do not patch the Waypoint source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/pingdotgg/waypoint/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Waypoint triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Waypoint user is \
having a problem with their install and started this session with \`waypoint triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Waypoint triage context

Generated by \`waypoint triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/pingdotgg/waypoint
