...
const status = {
  ...
  installed: true,
  current: true,
  unitPath: "/home/me/.config/systemd/user/waypoint.service",
  logPath: "/home/me/.waypoint/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Waypoint service",
      "  Status: installed · waypoint@0.0.29",
      "  Unit: /home/me/.config/systemd/user/waypoint.service",
      "  Logs: /home/me/.waypoint/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx waypoint@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx waypoint@0.0.32-nightly.1 service update");
  expect(output).not.toContain("waypoint@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx waypoint@0.0.29 service update");
  expect(output).not.toContain("npx waypoint@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "waypoint@0.0.32-nightly.1 (newer than this waypoint@0.0.31 CLI)");
  assert.include(output, "npx waypoint@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx waypoint@latest service update");
});
  ...
  it.effect.each(["install", "update"] as const)(
    ...
    (command) =>
      Effect.gen(function* () {
        ...
        const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-service-cli-test-" });
