...
import { HostProcessArguments } from "@waypoint/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `waypoint` command.
 */
...
/**
 * The `waypoint` package spec to suggest. The literal spec the user typed (e.g.
 * `waypoint@nightly`) is resolved away before our process starts, so re-derive it
...
function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "waypoint@nightly" : "waypoint";
}

/**
 * Render a `waypoint <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx waypoint connect` suggests
 * `npx waypoint serve`, a global install suggests `waypoint serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `waypoint ${input.subcommand}`;
  }
