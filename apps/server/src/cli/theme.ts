...
/**
 * `waypoint theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `waypoint theme set`.
 *
...
import { fromJsonStringPretty, fromLenientJson } from "@waypoint/shared/schemaJson";
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@waypoint/shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedError<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from Waypoint, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedError<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `waypoint theme clear` to remove the theme.";
  }
}

const envWaypointHome = Config.string("WAYPOINT_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then WAYPOINT_HOME,
  // then the default home. A provisioning script exporting WAYPOINT_HOME must
  // not have this one command silently target the default install.
  const envHome = Option.filter(yield* envWaypointHome, (value) => value.trim().length > 0);
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `waypoint theme set ocean` publish ./ocean whenever the cwd happens to
