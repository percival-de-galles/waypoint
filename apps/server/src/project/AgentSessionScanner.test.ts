...
import { symlinksSupported } from "@waypoint/shared/testing/symlinks";
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "waypoint-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const olderWorkspace = yield* makeTempDir("waypoint-workspace-older-");
        const newerWorkspace = yield* makeTempDir("waypoint-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const workspace = yield* makeTempDir("waypoint-workspace-");
        const otherWorkspace = yield* makeTempDir("waypoint-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const workspace = yield* makeTempDir("waypoint-workspace-");
        const linkParent = yield* makeTempDir("waypoint-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const backingUpper = yield* makeTempDir("waypoint-backing-upper-");
        const backingLower = yield* makeTempDir("waypoint-backing-lower-");
        const aliasParent = yield* makeTempDir("waypoint-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("waypoint-claude-legacy-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("waypoint-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("waypoint-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("waypoint-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("waypoint-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("waypoint-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const otherCodexHome = yield* makeTempDir("waypoint-codex-other-");
        const workspace = yield* makeTempDir("waypoint-workspace-");
        const otherWorkspace = yield* makeTempDir("waypoint-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-legacy-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("waypoint-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("waypoint-codex-env-");
        const claudeWorkspace = yield* makeTempDir("waypoint-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("waypoint-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("waypoint-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("waypoint-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("waypoint-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("waypoint-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and Waypoint data directory", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const configBaseDir = yield* makeTempDir("waypoint-scanner-base-");
        const workspace = yield* makeTempDir("waypoint-workspace-");
    ...
    it.effect("excludes Waypoint-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        ...
        const worktreeCwd = path.join(claudeHomePath, ".waypoint", "worktrees", "waypoint", "wt-1");
    ...
    it.effect("excludes Codex scratch directories and Downloads", () =>
      Effect.gen(function* () {
        ...
        const runId = path.basename(yield* makeTempDir("waypoint-scanner-test-"));
        ...
        const keep = yield* makeTempDir("waypoint-workspace-keep-");
    ...
    it.effect("skips linked git worktrees and reports the origin of real checkouts", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const repo = yield* makeTempDir("waypoint-workspace-repo-");
        const worktree = yield* makeTempDir("waypoint-workspace-worktree-");
        const plain = yield* makeTempDir("waypoint-workspace-plain-");
        const noRemote = yield* makeTempDir("waypoint-workspace-noremote-");
        const submodule = yield* makeTempDir("waypoint-workspace-submodule-");

        yield* fileSystem.makeDirectory(path.join(repo, ".git"));
        yield* fileSystem.writeFileString(
          path.join(repo, ".git", "config"),
          '[core]\n\tbare = false\n[remote "origin"]\n\turl = git@github.com:pingdotgg/waypoint.git\n\tfetch = +refs/heads/*:refs/remotes/origin/*\n',
        );
        yield* fileSystem.writeFileString(
        ...
        expect(
          ...
          {
            path: repo,
            git: { remoteKey: "github.com/pingdotgg/waypoint", repository: "pingdotgg/waypoint" },
          },
        ]);
      }),
    );

    it.effect("excludes sandboxes under the configured worktrees dir without .waypoint in the path", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        ...
        // dir contains no `.waypoint` segment — only the config-based prefix match
        // can exclude this one.
        const worktreeCwd = path.join(configBaseDir, "worktrees", "waypoint", "wt-2");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
        yield* writeTranscript({
          filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
          contents: claudeSessionLine(worktreeCwd),
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "waypoint", "wt-3");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("waypoint-metadata-home-");
        const secondHome = yield* makeTempDir("waypoint-metadata-second-");
        const codexHomePath = yield* makeTempDir("waypoint-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("waypoint-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("waypoint-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("waypoint-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("waypoint-short-metadata-codex-");
        const workspace = yield* makeTempDir("waypoint-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("waypoint-record-metadata-home-");
        const secondHome = yield* makeTempDir("waypoint-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("waypoint-record-metadata-codex-");
        const workspace = yield* makeTempDir("waypoint-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("waypoint-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("waypoint-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("waypoint-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("waypoint-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("waypoint-record-limit-codex-");
          const workspace = yield* makeTempDir("waypoint-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const customHome = yield* makeTempDir("waypoint-codex-custom-");
        const workspace = yield* makeTempDir("waypoint-workspace-");
    ...
    it.effect("streams large transcripts across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("waypoint-budget-claude-");
        const codexHomePath = yield* makeTempDir("waypoint-budget-codex-");
        const workspace = yield* makeTempDir("waypoint-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("waypoint-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("waypoint-record-budget-codex-");
        const workspace = yield* makeTempDir("waypoint-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("waypoint-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("waypoint-completed-claude-");
        const codexHomePath = yield* makeTempDir("waypoint-completed-codex-");
        const workspace = yield* makeTempDir("waypoint-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const workspace = yield* makeTempDir("waypoint-workspace-");
        const nonFilePath = yield* makeTempDir("waypoint-non-file-");
    ...
    it.effect("does not import sessions from a Waypoint-managed worktree", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        ...
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const configBaseDir = yield* makeTempDir("waypoint-scanner-base-");
        const workspace = path.join(configBaseDir, "worktrees", "waypoint", "managed-worktree");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const sharedHome = yield* makeTempDir("waypoint-codex-shared-");
        const workspace = yield* makeTempDir("waypoint-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("waypoint-claude-home-");
        const codexHomePath = yield* makeTempDir("waypoint-codex-home-");
        const oldWorkspace = yield* makeTempDir("waypoint-workspace-old-");
        const recentWorkspace = yield* makeTempDir("waypoint-workspace-recent-");
        const recentHome = yield* makeTempDir("waypoint-claude-recent-home-");
