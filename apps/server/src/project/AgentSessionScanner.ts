...
} from "@waypoint/shared/git";
import { HostProcessEnvironment, HostProcessPlatform } from "@waypoint/shared/hostProcess";
import { normalizeProjectPathForComparison } from "@waypoint/shared/path";
...
>()("waypoint/project/AgentSessionScanner") {}
...
/**
 * Waypoint runs its own agent sessions inside disposable worktrees. Their
 ...
 * worktrees directory plus the conventional `.waypoint/worktrees` layout, which
 * also catches sandboxes from other Waypoint homes on the same machine. Separators
...
function isWaypointManagedWorktree(
  ...
  return (
    ...
    normalized.includes("/.waypoint/worktrees/")
  );
}
...
export const make = Effect.gen(function* () {
  ...
  const isExcludedProjectPath = (candidatePath: string) =>
    ...
    isWaypointManagedWorktree(candidatePath, worktreesDir, foldWorktreeCase);
