...
import { CursorSettings, ProviderInstanceId } from "@waypoint/contracts";
...
const CursorTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "waypoint-cursor-text-generation-test-",
...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-cursor-text-acp-"));
...
it.layer(CursorTextGenerationTestLayer)("CursorTextGeneration", (it) => {
  it.effect("uses ACP model config options instead of raw CLI model ids", () => {
    const requestLogDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "waypoint-cursor-text-log-"),
    );
    ...
    return withFakeAcpAgent(
      {
        ...
        WAYPOINT_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("accepts json objects with extra assistant text around them", () =>
    withFakeAcpAgent(
      {
        WAYPOINT_ACP_PROMPT_RESPONSE_TEXT:
  ...
  it.effect.skipIf(HostProcessPlatform.defaultValue() === "win32")(
    ...
    () => {
      const exitLogDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "waypoint-cursor-text-exit-log-"),
      );
