...
import { ProviderDriverKind, ProviderInstanceId, type ServerProvider } from "@waypoint/contracts";
...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("waypoint-npm-capabilities");
  ...
  it.effect("proves Windows npm ownership from the package manifest beside the shim", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("waypoint-npm-windows-capabilities");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("waypoint-pnpm-capabilities");
        const link = linkIntoPackage(tempDir, "package-tool", [
          ".local",
          "share",
          "pnpm",
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("waypoint-bun-capabilities");
  ...
  it.effect.skipIf(windowsHost)("switches to native updates and runs the resolved executable", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("waypoint-native-capabilities");
  ...
  it.effect.skipIf(windowsHost)("runs an explicit native updater outside PATH", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("waypoint-native-update");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* makeTempDir("waypoint-homebrew-node-capabilities");
  ...
  it.effect.skipIf(windowsHost)("carries the native updater's environment into the action", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("waypoint-native-env");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("waypoint-missing-native-capabilities");
  ...
  it.effect.each([
    ...
    (fixture) =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("waypoint-homebrew-capabilities");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("waypoint-homebrew-foreign-prefix");
        const brewBinDir = NodePath.join(tempDir, "brew-bin");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("waypoint-custom-capabilities");
