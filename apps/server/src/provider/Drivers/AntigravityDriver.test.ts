...
const makeHarness = Effect.fn("makeAntigravityDriverHarness")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-antigravity-driver-" });
  ...
  const instance = yield* AntigravityDriver.create({
    ...
    environment: [
      ...
      { name: "WAYPOINT_ACP_ANTIGRAVITY", value: "1" },
      { name: "WAYPOINT_ACP_REQUEST_LOG_PATH", value: requestLog },
...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "waypoint-antigravity-driver-config-",
}).pipe(
