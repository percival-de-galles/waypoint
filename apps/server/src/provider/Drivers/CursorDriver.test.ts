...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "waypoint-cursor-driver-copy-command-",
}).pipe(
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-cursor-driver-" });
    ...
    Effect.gen(function* () {
      const instance = yield* CursorDriver.create({
        ...
        config: {
          ...
          binaryPath: NodePath.join(NodeOS.tmpdir(), "waypoint-cursor-missing", "cursor-agent"),
        },
      });
