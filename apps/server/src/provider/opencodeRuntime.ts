...
import type { ChatAttachment, ProviderApprovalDecision, RuntimeMode } from "@waypoint/contracts";
import {
...
import { compareSemverVersions, parseSemver } from "@waypoint/shared/semver";
...
export const verifyOpenCodeServerVersion = Effect.fn("verifyOpenCodeServerVersion")(function* (
    ...
    Effect.mapError(
      (cause) =>
        new OpenCodeRuntimeError({
          operation: "global.health",
          detail: `OpenCode server returned an invalid health response. Waypoint requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
          cause,
        }),
    ),
  );
  if (parseSemver(health.version) === null) {
    return yield* new OpenCodeRuntimeError({
      operation: "global.health",
      detail: `OpenCode server returned an invalid version. Waypoint requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
    });
  }
...
export class OpenCodeRuntime extends Context.Service<OpenCodeRuntime, OpenCodeRuntimeShape>()(
  "waypoint/provider/opencodeRuntime",
) {}
