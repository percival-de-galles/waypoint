...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@waypoint/contracts";
import { HostProcessExecutablePath, HostProcessPlatform } from "@waypoint/shared/hostProcess";
...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__WAYPOINT_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/waypoint-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by Waypoint. */
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which Waypoint points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The Waypoint runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and Waypoint browser-helper URLs without logging stderr. */
