...
const mockRuntimeOptions = {
  ...
  cwd: process.cwd(),
  clientInfo: { name: "waypoint-test", version: "0.0.0" },
  authMethodId: "test",
...
describe("AcpSessionRuntime", () => {
    ...
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...
        spawn: {
          ...
          env: { WAYPOINT_ACP_WAIT_FOR_RESUME_RELEASE: "1" },
        },
  ...
  it.effect("waits for native cancellation and drains final updates before another prompt", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...
        spawn: {
          ...mockRuntimeOptions.spawn,
          env: { WAYPOINT_ACP_COMPLETE_FIRST_PROMPT_ON_CANCEL: "1" },
        },
  ...
  it.effect("fails a pending request when the stderr handler rejects the runtime", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* AcpSessionRuntime.make({
        ...mockRuntimeOptions,
        spawn: { ...mockRuntimeOptions.spawn, env: { WAYPOINT_ACP_FLOOD_STDERR: "1" } },
  ...
  it.effect("does not restore ambient variables to a sanitized child environment", () =>
    Effect.gen(function* () {
      yield* Effect.acquireRelease(
        Effect.sync(() => {
          const previous = process.env.WAYPOINT_ACP_RUNTIME_AMBIENT;
          process.env.WAYPOINT_ACP_RUNTIME_AMBIENT = "sentinel";
          return previous;
        }),
        (previous) =>
          Effect.sync(() => {
            if (previous === undefined) delete process.env.WAYPOINT_ACP_RUNTIME_AMBIENT;
            else process.env.WAYPOINT_ACP_RUNTIME_AMBIENT = previous;
          }),
      );
      const runtime = yield* AcpSessionRuntime.make({
        ...mockRuntimeOptions,
        spawn: {
          ...
          args: mockAgentArgs,
          extendEnv: false,
          env: { WAYPOINT_ACP_RUNTIME_EXPLICIT: "kept" },
        },
      });
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              WAYPOINT_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "waypoint-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              WAYPOINT_ACP_EMIT_GENERIC_TOOL_PLACEHOLDERS: "1",
            },
          },
          cwd: process.cwd(),
          clientInfo: { name: "waypoint-test", version: "0.0.0" },
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              WAYPOINT_ACP_FAIL_LOAD_SESSION: "1",
            },
          },
          cwd: process.cwd(),
  ...
  it.effect("completes session/load after replay becomes idle while its RPC stays pending", () =>
    Effect.gen(function* () {
      ...
      expect(started.sessionSetupResult._meta).toMatchObject({
        waypointSessionLoadReady: "replay_idle",
      });
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              WAYPOINT_ACP_HANG_LOAD_SESSION_AFTER_REPLAY: "1",
              WAYPOINT_ACP_LOAD_SESSION_DELAY_MS: "10000",
            },
          },
          cwd: process.cwd(),
          resumeSessionId: "mock-session-1",
