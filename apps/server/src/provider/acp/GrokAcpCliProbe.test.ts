 ...
 * Enable with: WAYPOINT_GROK_ACP_PROBE=1 vp test run GrokAcpCliProbe
 * Set WAYPOINT_GROK_LIVE_TURN=1 to also send a small prompt to the real model.
 *
...
const makeProbeRuntime = Effect.gen(function* () {
  ...
  return yield* makeGrokAcpRuntime({
    ...
    childProcessSpawner,
    cwd: process.cwd(),
    clientInfo: { name: "waypoint-grok-probe", version: "0.0.0" },
  });
});

describe.runIf(process.env.WAYPOINT_GROK_ACP_PROBE === "1")("Grok ACP CLI probe", () => {
  ...
  it.effect.skipIf(process.env.WAYPOINT_GROK_LIVE_TURN !== "1")(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* runtime.prompt({
          prompt: [{ type: "text", text: "Reply exactly GROK_WAYPOINT_OK. Do not use any tools." }],
        });
        yield* runtime.drainEvents;
        expect(result.stopReason).toBe("end_turn");
        expect(chunks.join("")).toContain("GROK_WAYPOINT_OK");
