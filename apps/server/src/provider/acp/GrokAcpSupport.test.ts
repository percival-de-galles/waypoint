...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no Waypoint runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when Waypoint is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the Waypoint referrer through Grok OAuth env", () => {
