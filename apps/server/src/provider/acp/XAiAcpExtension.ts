...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@waypoint/contracts";
...
/**
 * Client captured the plan for Waypoint's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via Waypoint UI.
 */
 ...
 * Used so Waypoint can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `waypoint-xai-prompt-${nextPromptFallbackId}`;
    });
