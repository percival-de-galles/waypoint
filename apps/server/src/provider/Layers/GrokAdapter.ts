...
import { stableStringify } from "@waypoint/shared/relaySigning";
...
export function selectGrokPermissionOptionId(
  ...
  // Grok 4.6 often omits allow_always. Waypoint still offers "Always allow this session".
...
export function makeGrokAdapter(grokSettings: GrokSettings, options?: GrokAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    /** Surface Grok plan.md as Waypoint's proposed-plan card (while writing + on exit). */
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const started = yield* Effect.gen(function* () {
            ...
            // Capture plan into Waypoint proposed-plan UI and abandon the native gate so
