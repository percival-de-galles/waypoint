...
import { parseCliArgs } from "@waypoint/shared/cliArgs";
...
} from "@waypoint/shared/claudeCompaction";
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      case "model_refusal_fallback":
        ...
        return;
      // Inner protocol/UX details with no Waypoint surface today — consumed
  ...
  const handleSdkMessage = Effect.fn("handleSdkMessage")(function* (
    ...
    // Wire-only command bookkeeping has no user-facing Waypoint lifecycle.
    ...
    switch (message.type) {
      ...
      case "rate_limit_event":
        ...
        return;
      // Composer prompt suggestions have no Waypoint surface; consumed deliberately.
      // `conversation_reset` announces a CLI-side conversation id swap
      // (e.g. /clear); Waypoint keeps its own thread identity and resume cursor.
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const handleAskUserQuestion = Effect.fn("handleAskUserQuestion")(function* (
        ...
        // expected lookup key. See the upstream report:
        // https://github.com/pingdotgg/t3code/issues/2388
      ...
      const handleResumeDialog = Effect.fn("handleResumeDialog")(function* (
        ...
        // The question copy lives in @waypoint/shared/claudeCompaction because
      ...
      // A permission launch arg is folded into the mode Waypoint sends rather than
