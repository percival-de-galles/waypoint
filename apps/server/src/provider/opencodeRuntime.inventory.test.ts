...
it.layer(testLayer)("OpenCodeRuntime inventory", (it) => {
  ...
  it.effect("drops oversized CLI skill output without losing the model inventory", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-opencode-inventory-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          isWindows
            ? '"%WAYPOINT_TEST_NODE_BINARY%" "%WAYPOINT_TEST_OPENCODE_SCRIPT%" %*'
            : 'exec "$WAYPOINT_TEST_NODE_BINARY" "$WAYPOINT_TEST_OPENCODE_SCRIPT" "$@"',
          "",
        ].join("\n"),
      );
      if (!isWindows) {
      ...
      const inventory = yield* runtime.loadInventoryFromCli({
        ...
        environment: {
          ...hostEnvironment,
          WAYPOINT_TEST_NODE_BINARY: executablePath,
          WAYPOINT_TEST_OPENCODE_SCRIPT: scriptPath,
        },
      });
