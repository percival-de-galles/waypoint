...
import { AssetPreviewTypeValidationError, ThreadId } from "@waypoint/contracts";
import { PROJECT_FAVICON_FALLBACK_MARKER } from "@waypoint/shared/projectFavicon";
...
const configLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "waypoint-asset-access-test-",
});
...
describe("AssetAccess", () => {
  it.effect("issues exact URLs for media and browser documents outside the workspace", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-root-" });
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-outside-" });
  ...
  it.effect("reports pixel dimensions from an image header and nothing for other files", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-dimensions-" });
  ...
  it.effect("resolves relative media paths from the thread workspace, including outside it", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-relative-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-validation-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-symlink-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-open-file-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-open-race-" });
        const filePath = path.join(root, "recording.mp4");
        const secretPath = path.join(root, "secret.txt");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-open-rejected-" });
        const filePath = path.join(root, "recording.mp4");
        const secretPath = path.join(root, "secret.txt");
        yield* fs.writeFileString(filePath, "video");
        yield* fs.writeFileString(secretPath, "secret");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-parent-race-" });
  ...
  it.effect("keeps in-place edits readable but requires a new URL after atomic replacement", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-media-replacement-" });
      const filePath = path.join(root, "recording.mp4");
  ...
  it.effect("issues workspace URLs that resolve the entry file and sibling assets", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-workspace-",
      });
  ...
  it.effect("rejects workspace files outside the authorized root", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-root-",
      });
      const outside = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-outside-",
      });
  ...
  it.effect("preserves non-missing canonical path failures when issuing asset URLs", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-permission-root-",
      });
      const htmlPath = path.join(root, "report.html");
  ...
  it.effect("issues exact workspace URLs for image previews", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-image-workspace-",
      });
  ...
  it.effect("issues project favicon capabilities for a saved override", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-override-",
      });
  ...
  it.effect("issues an exact capability for a saved favicon outside the workspace", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-workspace-",
      });
      const pictures = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-pictures-",
      });
  ...
  it.effect("ignores a client favicon path hint", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-hint-",
      });
      yield* fileSystem.makeDirectory(path.join(root, "brand"));
  ...
  it.effect("keeps automatic favicon resolution separate from a saved override", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-automatic-",
      });
      yield* fileSystem.makeDirectory(path.join(root, "brand"));
      yield* fileSystem.writeFileString(path.join(root, "brand", "saved.svg"), "<svg>saved</svg>");
  ...
  it.effect("rejects a resolved project favicon with a non-image extension", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-type-",
      });
  ...
  it.effect("buckets project favicon expiry after content hashing", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-expiry-",
      });
  ...
  it.effect("preserves structured project favicon resolution causes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-asset-favicon-error-",
      });
