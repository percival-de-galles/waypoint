  ...
  readonly body: {
    readonly batch?: ReadonlyArray<{
      ...
      readonly properties?: {
        ...
        readonly waypointCodeVersion?: string;
      };
    }>;
  } | null;
}
...
it.layer(NodeServices.layer)("AnalyticsService test", (it) => {
  it.effect("flush drains all buffered events across multiple batches", () =>
    Effect.gen(function* () {
      ...
      const serverConfigLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
        prefix: "waypoint-telemetry-base-",
      });

      const telemetryLayer = AnalyticsService.layer.pipe(Layer.provideMerge(serverConfigLayer));
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromUnknown({
          WAYPOINT_TELEMETRY_ENABLED: true,
          WAYPOINT_POSTHOG_KEY: "phc_test_key",
          WAYPOINT_POSTHOG_HOST: "http://localhost",
          WAYPOINT_TELEMETRY_FLUSH_BATCH_SIZE: 20,
        }),
      );
        ...
        batchRequests.every((request) =>
          request.body.batch.every(
            (event) =>
              ...
              event.properties.serverAppVersion === event.properties.waypointCodeVersion &&
  ...
  it.effect("does not send batch requests when telemetry is disabled", () =>
    Effect.gen(function* () {
      ...
      const serverConfigLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
        prefix: "waypoint-telemetry-disabled-",
      });
      const telemetryLayer = AnalyticsService.layer.pipe(Layer.provideMerge(serverConfigLayer));
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromUnknown({
          WAYPOINT_TELEMETRY_ENABLED: false,
          WAYPOINT_POSTHOG_KEY: "phc_test_key",
          WAYPOINT_POSTHOG_HOST: "http://localhost",
        }),
      );
