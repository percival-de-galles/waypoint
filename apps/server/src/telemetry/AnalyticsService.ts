...
import type { ClientOs } from "@waypoint/contracts";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("WAYPOINT_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("WAYPOINT_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("WAYPOINT_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("WAYPOINT_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.number("WAYPOINT_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
...
>()("waypoint/telemetry/AnalyticsService") {
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          waypointCodeVersion: packageJson.version,
          clientType,
