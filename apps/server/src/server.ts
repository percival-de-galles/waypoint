import { EnvironmentHttpApi, ProviderDriverKind } from "@waypoint/contracts";
...
import { disableTailscaleServe, ensureTailscaleServe } from "@waypoint/tailscale";
...
// Waypoint's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
        ...
        yield* forkParked(
          Effect.gen(function* () {
            ...
            yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`).pipe(
              ...
              Effect.tap(() => Effect.logInfo("Waypoint Connect desired link reconciled on startup")),
              Effect.catch((cause) =>
                Effect.logWarning("Failed to reconcile Waypoint Connect desired link on startup", {
