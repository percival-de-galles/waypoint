...
export interface TelemetryCounters {
  ...
  readonly allWaypoint: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allWaypoint: ResourceTelemetryAggregate;
  };
...
export const emptyTelemetryCounters = (): TelemetryCounters => ({
  ...
  allWaypoint: emptyGroupCounters(),
});
...
function applyLifecycleCounters(input: {
  ...
  let allWaypoint = input.counters.allWaypoint;
  for (const processDelta of input.deltas) {
    ...
    allWaypoint = incrementCounters(allWaypoint, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allWaypoint = incrementCounters(allWaypoint, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allWaypoint = incrementCounters(allWaypoint, { processExits: 1 });
  }

  return { backend, electron, monitor, allWaypoint };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allWaypoint: aggregate(ordered, counters.allWaypoint),
    },
    deltas: processDeltas,
  };
}
