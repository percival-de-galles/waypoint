...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@waypoint/shared/git";
import { compactTraceAttributes } from "@waypoint/shared/observability";
import { decodeJsonResult } from "@waypoint/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `waypoint-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/waypoint/pre-refresh", headCommit],
