...
import { GitManagerError } from "@waypoint/contracts";
...
const remoteStatusWithPr: VcsStatusRemoteResult = {
  ...
  pr: {
    ...
    url: "https://github.com/pingdotgg/waypoint/pull/2978",
    baseRef: "main",
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "waypoint-vcs-auto-pull-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "waypoint-vcs-auto-pull-link-",
        });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "waypoint-vcs-status-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "waypoint-vcs-status-link-",
        });
