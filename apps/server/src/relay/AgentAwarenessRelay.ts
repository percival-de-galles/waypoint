...
import { projectThreadAwareness } from "@waypoint/shared/agentAwareness";
import { makeDrainableWorker } from "@waypoint/shared/DrainableWorker";
...
>()("waypoint/relay/AgentAwarenessRelay") {}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
  ...
  const payload = {
    iss: `waypoint-env:${input.environmentId}`,
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for Waypoint Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo("agent activity publishing disabled by Waypoint Connect configuration");
          break;
        case "enabled":
