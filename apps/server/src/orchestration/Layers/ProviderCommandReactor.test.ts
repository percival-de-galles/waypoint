...
import { serializeAssistantCitation } from "@waypoint/shared/assistantCitations";
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir =
      input?.baseDir ?? NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-reactor-"));
  ...
  it("pins the first user message when regeneration context is truncated", async () => {
    ...
    expect(message).not.toContain("waypoint-citation://");
  ...
  it("matches the client-seeded title even when the outgoing prompt is reformatted", async () => {
    ...
    expect(harness.generateThreadTitle.mock.calls[0]?.[0].message).not.toContain("waypoint-citation://");
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
  ...
  it("generates a worktree branch name for the first turn", async () => {
    ...
    expect(harness.generateBranchName.mock.calls[0]?.[0].message).not.toContain("waypoint-citation://");
