...
function createGitRepository() {
  const cwd = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "waypoint-checkpoint-handler-"));
...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
      prefix: "waypoint-checkpoint-reactor-test-",
    });
  ...
  it("re-asks for the pull request at turn end when the thread branch is checked out", async () => {
    ...
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "waypoint/feature",
      localStatusRefName: "waypoint/feature",
  ...
  it("re-asks for the pull request after adopting a drifted checkout", async () => {
    ...
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "waypoint/original-branch",
      localStatusRefName: "waypoint/renamed-by-agent",
      pullRequestRefreshCalls,
    });
    ...
    await waitForEvent(
      ...
      (event) =>
        ...
        (event as unknown as { payload: { branch?: string } }).payload.branch ===
          "waypoint/renamed-by-agent",
    );

    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("waypoint/renamed-by-agent");
  });
  ...
  it("follows a checkout from a saved placeholder branch and refreshes its pull request", async () => {
    ...
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "waypoint/fd9cbe0e",
  ...
  it.each(["waypoint/original-branch", "waypoint/fd9cbe0e"])(
  ...
  it("does not adopt a temporary placeholder checkout as the thread branch", async () => {
    const harness = await createHarness({
      seedFilesystemCheckpoints: false,
      threadBranch: "waypoint/original-branch",
      localStatusRefName: "waypoint/0a1b2c3d",
    });
    ...
    const snapshot = await harness.readModel();
    const thread = snapshot.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.branch).toBe("waypoint/original-branch");
  });
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "waypoint-checkpoint-runtime-non-repo-"),
    );
