 ...
 * agent-device daemon in HTTP mode under a Waypoint-owned state directory. Both are
 ...
 *
 * The hub runs in its standalone mode (origin root). The Waypoint proxy strips its
...
import { waitForHttpReady } from "@waypoint/shared/httpReadiness";
...
import { isCommandAvailable } from "@waypoint/shared/shell";
