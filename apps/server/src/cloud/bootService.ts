...
const BOOT_SERVICE_NAME = "waypoint";
const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
// `.service` suffix keeps the label distinct from the desktop app's bundle id
// (dev.waypoint.app), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = "dev.waypoint.app.service";
const BOOT_SERVICE_PLIST_FILE = `${BOOT_SERVICE_LAUNCHD_LABEL}.plist`;
const BOOT_SERVICE_UNIT_ENV = "WAYPOINT_BOOT_SERVICE_UNIT";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=WAYPOINT_HOME=${quoteSystemdValue(plan.baseDir)}`,
    ...(plan.npmPackage
      ? [`Environment=WAYPOINT_NPM_PACKAGE=${quoteSystemdValue(plan.npmPackage)}`]
      : []),
...
export function renderBootServicePlist(
  ...
  return [
    ...
    `    <key>WAYPOINT_HOME</key>`,
    `    <string>${escapeXmlText(plan.baseDir)}</string>`,
    ...(plan.npmPackage
      ? [
          `    <key>WAYPOINT_NPM_PACKAGE</key>`,
          `    <string>${escapeXmlText(plan.npmPackage)}</string>`,
        ]
      : []),
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Waypoint background service.";
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    case "user-manager-unavailable":
      return "Cannot reach the systemd user manager. Run `systemctl --user status` in a login session for the service user. Install your distribution's systemd user-session support if it is missing; do not run Waypoint with sudo.";
    ...
    case "linger-disabled":
      return 'Lingering is disabled. Waypoint will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `waypoint service update` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status waypoint.service`, then run `waypoint service update`.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace waypoint@${this.installedVersion} with older waypoint@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  const npmPackage = yield* Config.string("WAYPOINT_NPM_PACKAGE").pipe(
    Config.withDefault(""),
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned waypoint runtime",
                  cause,
                }),
            ),
