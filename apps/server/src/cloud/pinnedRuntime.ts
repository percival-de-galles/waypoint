...
/**
 * A pinned runtime is an exact `waypoint@<version>` npm-installed into
 ...
 * switching over, never `npx waypoint`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "waypoint", "dist", "bin.mjs"),
...
/**
 * Installs `waypoint@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const packageName = yield* Config.string("WAYPOINT_NPM_PACKAGE").pipe(
    Config.withDefault(""),
    Effect.map((value) => value.trim()),
    Effect.mapError(
      (cause) => new PinnedRuntimeInstallError({ step: "reading WAYPOINT_NPM_PACKAGE", cause }),
    ),
  );
  if (!packageName) {
    return yield* new PinnedRuntimePreflightBlockedError({
      ...
      reason:
        "Waypoint package installation is not configured. Run your source build directly, or set WAYPOINT_NPM_PACKAGE to a package you own before installing a service or updating it.",
    });
  }
  if (versionDirExists) {
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "waypoint", "dist", "bin.mjs"),
    sentinelPath: input.path.join(stagingDir, ".install-complete"),
  };

  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned waypoint runtime (this can take a few minutes)";
    const installArgs = [
      "install",
      "--prefix",
      stagingDir,
      "--no-fund",
      "--no-audit",
      packageName === "waypoint"
        ? `waypoint@${input.version}`
        : `waypoint@npm:${packageName}@${input.version}`,
    ];
    yield* runner
      .run({
        command: "npm",
