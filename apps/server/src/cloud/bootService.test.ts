...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.waypoint/runtime/service-launcher.mjs",
    baseDir: "/home/theo/.waypoint",
    logPath: "/home/theo/.waypoint/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/waypoint.service",
  });

  expect(unit).toContain(
    "ExecStart=/usr/bin/node /home/theo/.waypoint/runtime/service-launcher.mjs",
  );
...
const macPlan = {
  ...
  launcherPath: "/Users/theo/.waypoint/runtime/service-launcher.mjs",
  baseDir: "/Users/theo/.waypoint",
  logPath: "/Users/theo/.waypoint/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/dev.waypoint.app.service.plist",
};
...
it("keeps launchd pinned to the stable launcher rather than a versioned server", () => {
  ...
  expect(plist).toContain("<string>/Users/theo/.waypoint/runtime/service-launcher.mjs</string>");
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.waypoint/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
it("escapes XML in host paths", () => {
  const plist = BootService.renderBootServicePlist(
    { ...macPlan, baseDir: "/Users/theo/Waypoint & <Co>" },
    { homeDir: "/Users/theo", environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin" },
  );

  expect(plist).toContain("<string>/Users/theo/Waypoint &amp; &lt;Co&gt;</string>");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-boot-service-test-" });
  const baseDir = path.join(home, ".waypoint");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable waypoint.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart waypoint.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop waypoint.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop waypoint.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now waypoint.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [
                "systemctl --user stop waypoint.service",
                "systemctl --user restart waypoint.service",
              ]
            : [
                "launchctl bootout --wait gui/501/dev.waypoint.app.service",
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "dev.waypoint.app.service.plist"),
        ),
      ).toBe(true);
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/dev.waypoint.app.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/dev.waypoint.app.service",
        "launchctl enable gui/501/dev.waypoint.app.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/dev.waypoint.app.service";
