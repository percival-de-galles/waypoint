...
import { RelayClientTracer } from "@waypoint/shared/relayTracing";
...
import type { RelayLinkProofRequest } from "@waypoint/contracts/relay";
  ...
  it.effect("does not accept an unauthenticated request trace parent", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("https://environment.example.test/api/waypoint-cloud/mint-credential", {
          headers: {
...
describe("releaseManagedTunnelOnShutdown", () => {
  ...
  const provideReleaseHarness =
    ...
    <A, E, R>(effect: Effect.Effect<A, E, R>) =>
      effect.pipe(
        ...
        Effect.provide(
          ServerConfigModule.layerTest("/", { prefix: "waypoint-http-release-test-" }).pipe(
  ...
  it.effect("still releases a pending update when the launcher is stopping", () => {
    // `waypoint service uninstall` or `systemctl stop` during the pending window:
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not waypoint_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("waypoint_relay"))).toBe(false);
  });
