  ...
  ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.gen(function* () {
        ...
        const entry = path.join(stagingDir, "node_modules", "waypoint", "dist", "bin.mjs");
...
it.layer(
  Layer.merge(
    NodeServices.layer,
    ConfigProvider.layer(ConfigProvider.fromEnv({ env: { WAYPOINT_NPM_PACKAGE: "waypoint" } })),
  ),
)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("refuses an unconfigured package before touching an incomplete installation", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-unpublished-" });
      const paths = pinnedRuntimePaths(path, baseDir, "1.2.3");
      yield* fs.makeDirectory(path.dirname(paths.entryPath), { recursive: true });
      yield* fs.writeFileString(paths.entryPath, "keep this incomplete build");
      const result = yield* ensurePinnedRuntimeInstalled({
        ...
        runner: ProcessRunner.ProcessRunner.of({
          run: () => Effect.die("Must not download an unconfigured package"),
        }),
      ...
      assert.equal(yield* fs.readFileString(paths.entryPath), "keep this incomplete build");
    }),
  );

  it.effect("installs an owned scoped package under the stable waypoint runtime path", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-scoped-package-" });
      ...
      const paths = yield* ensurePinnedRuntimeInstalled({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({ env: { WAYPOINT_NPM_PACKAGE: "@example/waypoint" } }),
          ),
        ),
      );
      assert.include(commands[0]!.args, "waypoint@npm:@example/waypoint@1.2.3");
  ...
  it.effect("installs through pnpm when its Node runtime has no npm executable", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-pinned-pnpm-" });
  ...
  it.effect("does not try a different installer for npm permission failures", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-pinned-permission-" });
  ...
  it.effect("validates a staging tree before atomically publishing it", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "waypoint-pinned-runtime-test-",
      });
  ...
  it.effect("replaces an incomplete pinned runtime", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "waypoint-pinned-runtime-repair-",
      });
      const finalPaths = pinnedRuntimePaths(path, baseDir, "1.2.3");
  ...
  it.effect("removes staging when installation is interrupted", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "waypoint-pinned-runtime-interrupt-",
      });
