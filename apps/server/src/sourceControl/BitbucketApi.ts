...
import { sanitizeBranchFragment } from "@waypoint/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@waypoint/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("WAYPOINT_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("WAYPOINT_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("WAYPOINT_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("WAYPOINT_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("waypoint/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `waypoint/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set WAYPOINT_BITBUCKET_EMAIL and WAYPOINT_BITBUCKET_API_TOKEN, or WAYPOINT_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
