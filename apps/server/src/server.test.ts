...
import { HostProcessEnvironment, HostProcessPlatform } from "@waypoint/shared/hostProcess";
...
import {
  ...
  type DpopPublicJwk,
} from "@waypoint/shared/dpop";
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@waypoint/shared/relayJwt";
import * as RelayClient from "@waypoint/shared/relayClient";
...
import * as WaypointProjectFileLoader from "./project/WaypointProjectFileLoader.ts";
  ...
  Effect.gen(function* () {
    ...
    yield* Layer.build(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "waypoint-web",
          attributes: {
            "service.runtime": "waypoint-web",
...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const tempBaseDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-router-test-" });
    ...
    const config: ServerConfig.ServerConfig["Service"] = {
      ...
      otlpServiceName: "waypoint-server",
      mode: "desktop",
      port: 0,
      host: "127.0.0.1",
      cwd: process.cwd(),
    ...
    const workspaceAndProjectServicesLayer = Layer.mergeAll(
      ...
      ProjectFaviconResolver.layer.pipe(
        Layer.provide(WorkspacePaths.layer),
        Layer.provide(WaypointProjectFileLoader.layer),
      ),
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected Waypoint Connect CLI authorization request.")),
...
const makeCloudMintCredentialRequest = (input: {
  ...
  const payload = {
    ...
    aud: input.audience ?? `waypoint-env:${input.environmentId}`,
...
const crossOriginClientOrigin = "http://remote-client.test:4773";
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  it.effect("parks HTTP ingress until command readiness", () =>
    Effect.gen(function* () {
      ...
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-router-gate-" });
  ...
  it.effect("serves static index content for GET / when staticDir is configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-router-static-" });
  ...
  it.effect("revalidates static files without sending unchanged bodies", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-static-cache-" });
  ...
  it.effect("serves changed HTML with the same size and timestamp", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-static-html-" });
      const indexPath = path.join(staticDir, "index.html");
  ...
  it.effect("caches hashed static assets without freezing mutable files or SPA fallbacks", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-static-hashes-" });
    ...
    it.effect(`revalidates hash-like static filenames with a ${manifest.label} manifest`, () =>
      Effect.gen(function* () {
        ...
        const staticDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "waypoint-static-mutable-",
        });
        yield* fileSystem.makeDirectory(path.join(staticDir, "assets"));
  ...
  it.effect("binds static metadata and bytes to one file across atomic replacement", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-static-replace-" });
  ...
  it.effect("closes static file handles after GET, HEAD, 304, and request cancellation", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-static-close-" });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/waypoint/environment");
  ...
  it.effect("reports unauthenticated session state without requiring auth", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(body.auth.sessionCookieName.startsWith("waypoint_session_"));
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("migrates a valid legacy remote-web session cookie", () =>
    Effect.gen(function* () {
      ...
      const legacyCookie = currentCookie.replace(/^waypoint_session_[^=]+=/, "waypoint_session=");
      const sessionUrl = yield* getHttpServerUrl("/api/auth/session");
    ...
    (source) =>
      Effect.gen(function* () {
        ...
        const response = yield* fetchEffect(sessionUrl, {
          headers:
            source === "cookie"
              ? { cookie: `${sessionCookie}; waypoint_session=stale` }
              : { authorization: `Bearer ${sessionToken}`, cookie: "waypoint_session=stale" },
        });
        const body = yield* responseJsonEffect<{ readonly authenticated: boolean }>(response);
      ...
      Effect.gen(function* () {
        ...
        const tokenResponse = yield* fetchEffect(tokenUrl, {
          ...
          body: new URLSearchParams({
            ...
            subject_token_type: "urn:waypoint:params:oauth:token-type:environment-bootstrap",
      ...
      const linkProofResponse = yield* fetchEffect(linkProofUrl, {
        ...
        body: jsonRequestBody({
          ...
          endpoint: {
            ...
            // "manual" and "cloudflare_tunnel" are supported; "waypoint_relay" is not.
            providerKind: "waypoint_relay",
          },
          origin: {
    ...
    () =>
      Effect.gen(function* () {
        const installedRelayClient = {
          ...
          executablePath: "/tmp/waypoint/tools/cloudflared",
    ...
    Effect.gen(function* () {
      ...
      const relayConfigResponse = yield* fetchEffect(relayConfigUrl, {
        ...
        body: jsonRequestBody({
          ...
          environmentCredential: "waypointenv_test_credential",
  ...
  it.effect("rejects relay config replacement from a different cloud account", () =>
    Effect.gen(function* () {
      ...
      const firstResponse = yield* postRelayConfig("user_123", "waypointenv_first_credential");
      const replacementResponse = yield* postRelayConfig("user_456", "waypointenv_second_credential");
  ...
  it.effect("serves the documented Waypoint Connect mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const mintUrl = yield* getHttpServerUrl("/api/waypoint-connect/mint-credential");
  ...
  it.effect("serves signed Waypoint Connect environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const healthUrl = yield* getHttpServerUrl("/api/waypoint-connect/health");
    ...
    Effect.gen(function* () {
      ...
      const wrongAudience = yield* postMint(
        makeCloudMintCredentialRequest({
          ...
          environmentId: testEnvironmentDescriptor.environmentId,
          clientProofKeyThumbprint: "client-proof-key-thumbprint",
          audience: "waypoint-env:other-environment",
  ...
  for (const desktopOrigin of ["waypoint://app", "waypoint-dev://app"]) {
        ...
        resourceSpans: [
          {
            resource: {
              attributes: [
                {
                  key: "service.name",
                  value: { stringValue: "waypoint-web" },
                },
              ],
            },
            scopeSpans: [
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "waypoint-web");
  ...
  it.effect("returns scanner skip counts over websocket rpc", () =>
    Effect.gen(function* () {
      ...
      const codexHome = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-agent-import-rpc-codex-",
      });
      const workspaceRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "waypoint-agent-import-rpc-workspace-",
      });
  ...
  it.effect("serves absolute host media without a local thread and rejects relative media", () =>
    Effect.gen(function* () {
      ...
      const fileSystem = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const directory = yield* fileSystem.makeTempDirectoryScoped({ prefix: "waypoint-host-media-" });
      const wsUrl = yield* getWsServerUrl("/ws");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-ws-auth-required-" });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-ws-project-search-" });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
      );
  ...
  it.effect("routes websocket rpc projects.listEntries and projects.readFile", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-ws-project-files-" });
  ...
  it.effect("routes websocket rpc projects.searchEntries excludes gitignored files", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "waypoint-ws-project-search-gitignored-",
      });
  ...
  it.effect.skipIf(!symlinksSupported)("preserves structured workspace rpc failures", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "waypoint-ws-workspace-errors-",
      });
      const outsideDir = yield* fs.makeTempDirectoryScoped({
        prefix: "waypoint-ws-workspace-errors-outside-",
      });
  ...
  it.effect("reports workspace root stat failures without relabeling them as missing", () =>
    Effect.gen(function* () {
      ...
      const blockedRoot = yield* fs.makeTempDirectoryScoped({
        prefix: "waypoint-ws-workspace-stat-error-",
      });
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-ws-project-write-" });
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const parentDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-ws-project-create-" });
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "waypoint/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "waypoint/bootstrap-refName",
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          newRefName: "waypoint/bootstrap-refName",
          baseRefName: "main",
          path: null,
        });
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const reportPath = yield* Config.string("WAYPOINT_TRANSFER_BUDGET_REPORT_PATH").pipe(
        Config.option,
      );
      ...
      const resultPath = yield* Config.string("WAYPOINT_TRANSFER_BUDGET_RESULT_PATH").pipe(
        Config.option,
      );
