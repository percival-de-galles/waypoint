...
const startupDependencies = Layer.mergeAll(
  ...
  Layer.succeed(
    ...
    HttpServer.HttpServer.of({
      address: { _tag: "TcpAddress", hostname: "127.0.0.1", port: 4773 },
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "waypoint-orphaned-provider-session-startup-",
...
it.effect.each(["opt-in desktop restart", "marked remote update"] as const)(
  ...
  (restart) =>
    Effect.gen(function* () {
      ...
      Effect.provide(
        ServerConfig.layerTest(process.cwd(), { prefix: "waypoint-restart-newer-turn-" }).pipe(
