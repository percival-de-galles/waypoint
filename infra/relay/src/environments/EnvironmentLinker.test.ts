...
import { RELAY_LINK_PROOF_TYP } from "@waypoint/shared/relayJwt";
...
const makeRequest = Effect.gen(function* () {
  ...
  const payload = {
    iss: "waypoint-env:env-link-test",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(EnvironmentCredentials.EnvironmentCredentials, {
          create: () => Effect.succeed("waypointenv_credential_secret"),
...
describe("EnvironmentLinker", () => {
  it.effect("uses verified JWT claims when linking an environment", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(result.environmentCredential).toBe("waypointenv_credential_secret");
  ...
  it.effect("links a publish-only environment with a non-secure nominal endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(persistedEndpoint).toBe("http://127.0.0.1:4773/");
