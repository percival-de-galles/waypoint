...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("waypoint-relay-traces", "prod")).toBe(
      "waypoint-relay-traces-prod",
    );
    expect(relayResourceNameForStage("waypoint-relay-traces", "dev_julius")).toBe(
      "waypoint-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "waypointrelay.com", hash)).toBe(
      "prod-abcdef0123456789.waypointrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "waypointrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
