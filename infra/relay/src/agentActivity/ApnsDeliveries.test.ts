  ...
  it.effect("queues Live Activity jobs with the device's APNs routing", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* deliveries.sendForTarget({
        target: {
          ...target,
          bundle_id: "dev.waypoint.app.preview",
      ...
      expect(queuedJobs).toMatchObject([
        {
          payload: {
            ...
            target: {
              token: "activity-token",
              bundleId: "dev.waypoint.app.preview",
  ...
  it.effect("sends signed jobs to the device's APNs environment and bundle topic", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(requests[0]?.headers["apns-topic"]).toBe(
        "dev.waypoint.app.preview.push-type.liveactivity",
      );
    }).pipe(
      Effect.provide(
        makeLayer({
          attempts,
          currentTargets: [
            { ...target, bundle_id: "dev.waypoint.app.preview", aps_environment: "sandbox" },
          ],
    ...
    for (const changed of ["bundle", "environment", "legacy"] as const) {
      it.effect(`routes ${kind} using current registration with ${changed} job metadata`, () => {
        ...
        const payload = makeApnsDeliveryJobPayload({
          ...
          ...(changed === "legacy"
            ? {}
            : { bundleId: "dev.waypoint.app.dev", apsEnvironment: "sandbox" as const }),
        ...
        return Effect.gen(function* () {
          ...
          expect(requests[0]?.headers["apns-topic"]).toBe(
            `${changed === "bundle" ? "dev.waypoint.app.preview" : "dev.waypoint.app.dev"}${kind === "live_activity_update" ? ".push-type.liveactivity" : ""}`,
          );
        }).pipe(
          Effect.provide(
            makeLayer({
              ...
              currentTargets: [
                {
                  ...
                  bundle_id:
                    changed === "bundle" ? "dev.waypoint.app.preview" : "dev.waypoint.app.dev",
