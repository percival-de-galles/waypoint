...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    WAYPOINT_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    WAYPOINT_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    WAYPOINT_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    WAYPOINT_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    WAYPOINT_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    WAYPOINT_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("WAYPOINT_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("WAYPOINT_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "WaypointRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Waypoint relay through Alchemy."));
