...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "waypoint-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "waypoint-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "WAYPOINT_MOBILE_OTLP_TRACES_DATASET=waypoint-mobile-traces-dev",
        "WAYPOINT_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_DATASET=waypoint-relay-client-traces-dev",
        "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
  ...
  it("replaces stale values while preserving unrelated entries", () => {
    expect(
      reconcileRootEnvPublicConfig(
        [
          "WAYPOINT_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "WAYPOINT_RELAY_URL=https://old.example.test",
          "WAYPOINT_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "WAYPOINT_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "WAYPOINT_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "WAYPOINT_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
