...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-waypoint", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "waypoint", specifier: pluginPath }],
