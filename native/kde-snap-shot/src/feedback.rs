...
const OBJECT: &str = "/com/waypointtools/KdeCapture/Feedback";
...
#[zbus::interface(name = "dev.waypoint.KdeCapture.Feedback")]
impl Bridge {
...
pub(super) fn run(connection: &Connection, directory: &Path, options: &str) -> Result<()> {
    ...
    let name = format!("waypoint-capture-feedback-{}", std::process::id());
...
mod tests {
    ...
    fn only_kwin_can_read_commands_or_acknowledge_feedback() {
        ...
        let unrelated = Proxy::new(
            ...
            destination.as_str(),
            OBJECT,
            "dev.waypoint.KdeCapture.Feedback",
        )
        .unwrap();
