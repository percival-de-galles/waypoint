...
const ANDROID_PACKAGE = "dev.waypoint.app";
const APP_SCHEME = "waypoint";
const IOS_READY_FILENAME = "WaypointShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/Waypoint.app",
);
...
const MOBILE_BUILD_ENV = {
  ...
  WAYPOINT_SHOWCASE_CAPTURE_BUILD: "1",
  JAVA_HOME:
...
Usage:
  pnpm --filter @waypoint/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/Waypoint.xcworkspace"),
      "-scheme",
      "Waypoint",
      "-configuration",
      "Debug",
...
async function captureIos(
  ...
  const scenePath = NodePath.join(
    ...
    "Library/Caches/WaypointShowcaseScene",
  );
  const readyPath = NodePath.join(
    await iosAppContainer(simulator.udid),
    "Library/Caches",
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/waypoint-showcase-ready && printf %s ${scene} > files/waypoint-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "waypoint-mobile-showcase-"),
  );
