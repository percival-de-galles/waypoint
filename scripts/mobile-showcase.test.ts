...
it("selects an explicit CI Android ABI without changing the local default", () => {
  ...
  assert.throws(() => resolveShowcaseAndroidAbi("mips"), /Unsupported WAYPOINT_SHOWCASE_ANDROID_ABI/u);
});
...
it("expands both appearances into independent upload-ready directories", () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      {
        appearance: "light",
        directory: NodePath.join("/captures", "apple", "iphone-test", "light", "waypoint"),
      },
      {
        appearance: "dark",
        directory: NodePath.join("/captures", "apple", "iphone-test", "dark", "waypoint"),
      },
    ],
  );
});
...
it("defaults every device to the app's own palette", () => {
  assert.equal(DEFAULT_SHOWCASE_THEME, "waypoint");
  assert.equal(
