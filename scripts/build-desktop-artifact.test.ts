...
const makeWindowsPayloadFixture = Effect.fn("test.makeWindowsPayloadFixture")(function* (input: {
  ...
  const tempDir = yield* fs.makeTempDirectoryScoped({
    prefix: "waypoint-windows-payload-test-",
  });
  ...
  yield* fs.writeFileString(
    path.join(resourcesDir, "resource-monitor/waypoint-resource-monitor.exe"),
    "monitor",
  );
  const appExecutableName = "waypoint.exe";
  ...
  if (input.wslRuntime !== undefined) {
    ...
    yield* fs.writeFileString(
      path.join(linuxPrebuildDir, "waypoint-wsl-node-pty.json"),
      '{"arch":"x64"}',
    );
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Waypoint (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Waypoint (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                WAYPOINT_DESKTOP_UPDATE_REPOSITORY: "pingdotgg/waypoint",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "pingdotgg/waypoint",
              },
            }),
          ),
        ),
  ...
  it.effect("omits update feeds for pull request preview builds", () =>
    Effect.gen(function* () {
      ...
      Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromEnv({ env: { GITHUB_REPOSITORY: "pingdotgg/waypoint" } }),
        ),
      ),
    ),
  );
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@waypoint/contracts": "workspace:*",
          "@waypoint/shared": "workspace:*",
          "@waypoint/ssh": "workspace:*",
          "@waypoint/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "Waypoint (Alpha) 1.2.3 Installer",
      ...
      // entry advertises MimeType=x-scheme-handler/waypoint; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Waypoint", schemes: ["waypoint", "waypoint-dev"] },
      ]);
    ...
    () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const tempDir = yield* fs.makeTempDirectoryScoped({
            prefix: "waypoint-windows-architecture-test-",
          });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const repoRoot = yield* fs.makeTempDirectoryScoped({
          prefix: "waypoint-resource-monitor-cache-test-",
        });
        const binaryPath = path.join(
          repoRoot,
          "native/resource-monitor/target/x86_64-unknown-linux-gnu/release/waypoint-resource-monitor",
        );
        ...
        yield* stageResourceMonitor({
          ...
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { WAYPOINT_DESKTOP_REUSE_RESOURCE_MONITOR: "true" },
              }),
            ),
          ),
        );
        ...
        assert.equal(
          yield* fs.readFileString(
            path.join(stageResourcesDir, "resource-monitor/waypoint-resource-monitor"),
          ),
          "cached monitor",
        );
      }),
  ...
  it.effect("reports missing Windows toolchain capabilities before building", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-windows-preflight-" });
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: {
                    ...
                    WAYPOINT_DESKTOP_REUSE_RESOURCE_MONITOR: "true",
                  },
                }),
              ),
            ),
  ...
  it.effect("rejects a PATH-discovered Python executable that is not Python 3", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-python2-preflight-" });
  ...
  it.effect("builds and stages native capture helpers for each Linux architecture", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const repoRoot = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-kde-stage-test-" });
        ...
        for (const backend of ["kde", "hyprland"] as const) {
          for (const [arch, target] of [
            ...
            const binary = path.join(
              ...
              target,
              `release/waypoint-${backend}-snap-shot`,
            );
            ...
            const installed = path.join(
              stageResourcesDir,
              `${backend}-capture/waypoint-${backend}-snap-shot`,
            );
  ...
  // The fixture's waypoint.exe is a text placeholder, not an executable. These
  ...
  it.effect("builds the Linux browser secret helper for a concrete architecture", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(helper.args.slice(-4), [
        ...
        "x64",
        "--output",
        path.join("/stage/resources", "browser-secret", "waypoint-browser-secret"),
      ]);
    }).pipe(
      Effect.provide(
        Layer.mergeAll(
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const resourceMonitorPath = path.join(
          fixture.packagedAppDir,
          "resources/resource-monitor/waypoint-resource-monitor.exe",
        );
        ...
        assert.deepStrictEqual(resourceMonitorError.missingFiles, [
          "resource-monitor/waypoint-resource-monitor.exe",
        ]);
      }),
    ),
  );
  ...
  it.effect("rejects a sidecar whose extracted server bundle cannot resolve", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fixture = yield* makeWindowsPayloadFixture({
          copyUnpackedNatives: true,
          serverEntrySource: 'import "waypoint-deliberately-missing-package";\n',
        });
        const error = yield* validateWindowsPackagedPayload({
          stageDistDir: fixture.stageDistDir,
          appExecutableName: fixture.appExecutableName,
        ...
        assert.include(error.output, "waypoint-deliberately-missing-package");
      }),
    ).pipe(Effect.provideService(HostProcessPlatform, "linux")),
  );
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const stageResourcesDir = yield* fs.makeTempDirectoryScoped({
          prefix: "waypoint-dmg-background-",
        });
  ...
  it.effect("fails clearly when the selected DMG background source is missing", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const stageResourcesDir = yield* fs.makeTempDirectoryScoped({
          prefix: "waypoint-dmg-background-missing-",
        });
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      WAYPOINT_APPLE_TEAM_ID: "abc1234567",
      WAYPOINT_MACOS_PROVISIONING_PROFILE: "/tmp/waypoint.provisionprofile",
      WAYPOINT_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });
    ...
    assert.deepStrictEqual(configuration, {
      ...
      provisioningProfilePath: "/tmp/waypoint.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      WAYPOINT_APPLE_TEAM_ID: "ABC1234567",
      WAYPOINT_MACOS_PROVISIONING_PROFILE: "/tmp/waypoint.provisionprofile",
      WAYPOINT_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.dev.waypoint.app</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      WAYPOINT_APPLE_TEAM_ID: "ABC1234567",
      WAYPOINT_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "WAYPOINT_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      WAYPOINT_APPLE_TEAM_ID: "ABC1234567",
      WAYPOINT_MACOS_PROVISIONING_PROFILE: "/tmp/waypoint.provisionprofile",
      WAYPOINT_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          WAYPOINT_APPLE_TEAM_ID: "ABC1234567",
          WAYPOINT_MACOS_PROVISIONING_PROFILE: "/tmp/waypoint.provisionprofile",
          WAYPOINT_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      WAYPOINT_APPLE_TEAM_ID: "ABC1234567",
      WAYPOINT_MACOS_PROVISIONING_PROFILE: "/tmp/waypoint.provisionprofile",
      WAYPOINT_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "WAYPOINT_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "dev.waypoint.app");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/waypoint.provisionprofile");
  ...
  it("stages the resource monitor as an external executable resource", () => {
    ...
    assert.equal(resourceMonitorExecutableName("mac"), "waypoint-resource-monitor");
    assert.equal(resourceMonitorExecutableName("win"), "waypoint-resource-monitor.exe");
  });
  ...
  it.effect("spawns tar with an archive target relative to the staged source tree", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const stageRoot = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-wsl-runtime-archive-" });
  ...
  it.effect("ships only Linux runtime members in the WSL archive", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-wsl-runtime-members-" });
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                WAYPOINT_DESKTOP_SKIP_BUILD: "true",
                WAYPOINT_DESKTOP_KEEP_STAGE: "true",
                WAYPOINT_DESKTOP_SIGNED: "true",
                WAYPOINT_DESKTOP_VERBOSE: "true",
                WAYPOINT_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
...
it.effect.skipIf(!symlinksSupported)("rebases packaged links into the isolated tree", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const root = yield* fs.makeTempDirectoryScoped({ prefix: "waypoint-copy-symlinks-" });
