...
import { resolveGitWorktreePath, resolveWorktreeWaypointHome } from "@waypoint/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@waypoint/shared/hostProcess";
...
export const DEFAULT_WAYPOINT_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".waypoint"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@waypoint/contracts",
    "--filter=@waypoint/web",
    "--filter=waypoint",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=waypoint", "dev"],
  "dev:web": ["run", "--filter=@waypoint/web", "dev"],
  "dev:desktop": ["run", "--filter=@waypoint/desktop", "--filter=@waypoint/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`WAYPOINT_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedError<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("WAYPOINT_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("WAYPOINT_PORT_OFFSET"),
  devInstance: optionalStringConfig("WAYPOINT_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "WAYPOINT_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `WAYPOINT_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric WAYPOINT_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed WAYPOINT_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_WAYPOINT_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly waypointHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .waypoint > ambient WAYPOINT_HOME) is resolved
    // by the caller; an unset waypointHome here genuinely means "use the default".
    const configuredBaseDir = waypointHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.WAYPOINT_HOME = resolvedBaseDir;
    } else {
      delete output.WAYPOINT_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed waypoint service (an
    // agent working inside Waypoint), these leak through and the child server
    // fails startup with "The service launcher started a different waypoint version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.WAYPOINT_SERVICE_LAUNCHER_CONTEXT;
    delete output.WAYPOINT_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.WAYPOINT_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.WAYPOINT_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.WAYPOINT_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.WAYPOINT_PORT = String(serverPort);
      ...
      delete output.WAYPOINT_SINGLE_ORIGIN_DEV;
      delete output.WAYPOINT_MODE;
      delete output.WAYPOINT_NO_BROWSER;
      delete output.WAYPOINT_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.WAYPOINT_HOST = host;
    }

    if (!isDesktopMode) {
      output.WAYPOINT_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.WAYPOINT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.WAYPOINT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.WAYPOINT_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.WAYPOINT_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.WAYPOINT_MODE = "web";
      delete output.WAYPOINT_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`WAYPOINT_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["WAYPOINT_PORT_OFFSET", "WAYPOINT_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.waypoint` — see @waypoint/shared/devHome for why this must
    // outrank an ambient WAYPOINT_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeWaypointHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedWaypointHome =
      (input.waypointHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.WAYPOINT_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      waypointHome: resolvedWaypointHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.WAYPOINT_HOME ?? (yield* DEFAULT_WAYPOINT_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.WAYPOINT_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.WAYPOINT_DEV_ALLOWED_ORIGINS = [
            env.WAYPOINT_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so WAYPOINT_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.WAYPOINT_BUNDLED_DEV === undefined) {
            env.WAYPOINT_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  waypointHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Waypoint data directory; runtime state is stored under userdata (equivalent to WAYPOINT_HOME). Inside a git worktree this defaults to that worktree's own .waypoint so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to WAYPOINT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("WAYPOINT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to WAYPOINT_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("WAYPOINT_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to WAYPOINT_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("WAYPOINT_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to WAYPOINT_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("WAYPOINT_PORT")),
  ),
