...
import { fromYaml } from "@waypoint/shared/schemaYaml";
import { HostProcessArchitecture, HostProcessPlatform } from "@waypoint/shared/hostProcess";
import { clerkFrontendApiHostnameFromPublishableKey } from "@waypoint/shared/relayAuth";
import { resolveSpawnCommand } from "@waypoint/shared/shell";
...
const DESKTOP_APP_ID = "dev.waypoint.app";
...
export function resourceMonitorExecutableName(platform: typeof BuildPlatform.Type): string {
  return platform === "win" ? "waypoint-resource-monitor.exe" : "waypoint-resource-monitor";
}
...
interface StagePackageJson {
  ...
  readonly waypointCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Waypoint always passes the user's installed Claude executable to the SDK,
...
export class InvalidAppleTeamIdError extends Schema.TaggedError<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `WAYPOINT_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedError<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "WAYPOINT_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedError<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "WAYPOINT_CLERK_PUBLISHABLE_KEY or WAYPOINT_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedError<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "WAYPOINT_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.WAYPOINT_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.WAYPOINT_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.WAYPOINT_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.WAYPOINT_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "WAYPOINT_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("WAYPOINT_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "WAYPOINT_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("WAYPOINT_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("WAYPOINT_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("WAYPOINT_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("WAYPOINT_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("WAYPOINT_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("WAYPOINT_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("WAYPOINT_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("WAYPOINT_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
  ...
  wslPrebuild: Config.string("WAYPOINT_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
...
export const preflightLinuxDesktopBuild = Effect.fn("preflightLinuxDesktopBuild")(function* (
  ...
  const reuseResourceMonitor = yield* Config.boolean("WAYPOINT_DESKTOP_REUSE_RESOURCE_MONITOR").pipe(
    Config.withDefault(false),
  );
  const reuseCaptureHelpers = yield* Config.boolean(
    "WAYPOINT_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS",
...
export const preflightWindowsDesktopBuild = Effect.fn("preflightWindowsDesktopBuild")(
  function* (input: { readonly arch: typeof BuildArch.Type; readonly bundlesWslRuntime: boolean }) {
    ...
    const reuseResourceMonitor = yield* Config.boolean(
      "WAYPOINT_DESKTOP_REUSE_RESOURCE_MONITOR",
    ).pipe(Config.withDefault(false));
...
const verifyPackagedBundleIsSelfContained = Effect.fn("verifyPackagedBundleIsSelfContained")(
  function* (input: { readonly asarPath: string; readonly verbose: boolean }) {
    ...
    const probeRoot = yield* fs.makeTempDirectoryScoped({
      prefix: "waypoint-bundle-selfcheck-",
    });
...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(function* (input: {
  ...
  const reuseHelpers = yield* Config.boolean("WAYPOINT_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS").pipe(
    Config.withDefault(false),
  );
  const binaryPath = path.join(
    ...
    rustTarget!,
    `release/waypoint-${input.backend}-snap-shot`,
  );
  if (!reuseHelpers) {
  ...
  const executable = path.join(destination, `waypoint-${input.backend}-snap-shot`);
  ...
  yield* runCommand(
    ChildProcess.make(
      ...
      [
        ...
        "--output",
        path.join(input.stageResourcesDir, "browser-secret", "waypoint-browser-secret"),
      ],
      { cwd: input.repoRoot },
    ),
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("WAYPOINT_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Waypoint (Nightly)"
    : (desktopPackageJson.productName ?? "Waypoint");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "Waypoint-${version}-${arch}.${ext}",
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "Waypoint captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "Waypoint",
          schemes: ["waypoint", "waypoint-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      ...
      // waypoint:// OAuth callbacks to the app.
      protocols: [
        {
          name: "Waypoint",
          schemes: ["waypoint", "waypoint-dev"],
...
// node-pty's prebuilds/linux-<arch>/ with a waypoint marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / WAYPOINT_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
  ...
  yield* fs.writeFileString(path.join(prebuildDir, "waypoint-wsl-node-pty.json"), `${markerJson}\n`);
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "waypoint-windows-primary-native-probe-",
  });
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  if (!(yield* isFile(resourceMonitorPath))) {
    return yield* new WindowsPackagedPayloadValidationError({
      ...
      packagedAppDir,
      missingFiles: ["resource-monitor/waypoint-resource-monitor.exe"],
    });
  }
    ...
    const requiredMembers = [
      ...
      ...(wslArch === undefined
        ...
        : [
            ...
            `node_modules/node-pty/prebuilds/linux-${wslArch}/waypoint-wsl-node-pty.json`,
          ]),
    ];
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `waypoint-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    waypointCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Waypoint desktop build",
    author: "Waypoint",
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @waypoint/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: WAYPOINT_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: WAYPOINT_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: WAYPOINT_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: WAYPOINT_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: WAYPOINT_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: WAYPOINT_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: WAYPOINT_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: WAYPOINT_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: WAYPOINT_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: WAYPOINT_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: WAYPOINT_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: WAYPOINT_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Waypoint."),
